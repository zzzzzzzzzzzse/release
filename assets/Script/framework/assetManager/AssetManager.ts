...
export class AssetManager {
            ...
            if ( cache ){
                ...
                if ( cache.isLoaded ){
                    ...
                    if (CC_DEBUG && cache.status == ResourceCacheStatus.WAITTING_FOR_RELEASE ){
                        cc.warn(this.logTag, `res:${path} suc`);
                    }
                    //加载完成
                    onComplete(cache);
                }else{
    ...
    private _onLoadComplete( cache : ResourceCacheData , completeCallback: (data: ResourceCacheData) => void,err:Error,data:cc.Asset | cc.Asset[]){
        ...
        let tempCache = cache;
        if (err) {
            cc.error(`${this.logTag}error:${cache.info.url} bec:${err.message ? err.message : "un"}`);
            cache.data = null;
            tempCache.data = null;
        ...
        if (cache.status == ResourceCacheStatus.WAITTING_FOR_RELEASE) {
            if (CC_DEBUG) cc.warn(this.logTag, `res:${cache.info.url}finich`);
            if (cache.data) {
                cache.status = ResourceCacheStatus.NONE;
        ...
        onComplete: (data:ResourceCacheData) => void): void {
            ...
            if ( cache ){
                ...
                if ( cache.isLoaded ){
                    //已经加载完成
                    if (CC_DEBUG && cache.status == ResourceCacheStatus.WAITTING_FOR_RELEASE ){
                        cc.warn(this.logTag, `res:${path} res`);
                    }
                    //加载完成
                    onComplete(cache);
                }else{
    ...
    public releaseAsset( info : ResourceInfo ){
        if ( info && info.bundle ){
            ...
            if( cache.isLoaded ){
                ...
                if( CC_DEBUG ) cc.warn(`${cache.info.url} res`);
            }
