        ...
        this._caches.forEach((value,key,origin)=>{
            if( Array.isArray(value.data) ){
                ...
                if( isAllDelete ){
                    ...
                    if( CC_DEBUG ) cc.log(`un bundle : ${this.name} dir : ${key}`);
                }
            }else{
                if( value.data && value.data.refCount == 0 ){
                    this._caches.delete(key);
                    if( CC_DEBUG ) cc.log(`un bundle : ${this.name} url : ${key}`);
                }
            }
        });
    }
...
class RemoteCaches {
    ...
    public retainAsset(info: ResourceInfo) {
        if (info && info.data) {
            ...
            if (cache) {
                if (cache.retain) {
                    if (!info.retain) {
                        if (CC_DEBUG) cc.warn(`res : ${info.url} `);
                    }
                } else {
    ...
    public remove(url: string) {
        ...
        if (cache && cache.data instanceof cc.Asset) {
            if (CC_DEBUG) cc.log(`unres:${cache.info.url}`);
            cache.data.decRef();
...
export class CacheManagerextend {
    private logTag = `[CacheManagerextend]: `;
    private static _instance: CacheManagerextend = null;
    public static Instance() {
        return this._instance || (this._instance = new CacheManagerextend());
    }
    ...
    public removeBundle( bundle : BUNDLE_TYPE ){
        ...
        if (bundleName && this._bundles.has(bundleName) ) {
            if( CC_DEBUG ) {
                cc.log(`bundle cache : ${bundleName}`)
                let data = this._bundles.get(bundleName);
                this._removeUnuseCaches();
                if( data.size > 0 ){
                    cc.error(`bundle ${bundleName} res`);
                }
            }
    ...
    private _getGetCacheByAsyncArgs(): { url: string, type: typeof cc.Asset, bundle: BUNDLE_TYPE } {
        if (arguments.length < 3) {
            if (CC_DEBUG) cc.error(`${this.logTag}`);
            return null;
        }
        if (typeof arguments[0] != "string") {
            if (CC_DEBUG) cc.error(`${this.logTag}string`);
            return null;
        }

        if (!cc.js.isChildClassOf(arguments[1], cc.Asset)) {
            if (CC_DEBUG) cc.error(`${this.logTag}cc.Asset`);
            return null;
        }
                ...
                if (cache.isLoaded) {
                    ...
                    if (_args.type) {
                        if (cache.data instanceof _args.type) {
                            ...
                            if (CC_DEBUG) cc.error(`${this.logTag}lx:${cc.js.getClassName(_args.type)}lex: ${cc.js.getClassName(cache.data)}t url : ${cache.info.url}`);
                            resolve(null);
                        }
                    } else {
                        resolve(cache.data);
                ...
                if (data && data instanceof args.type) {
                    ...
                    Manager.assetManager.load(args.bundle, args.url, args.type, null, (cache) => {
                        if (cache && cache.data && cache.data instanceof args.type) {
                            ...
                            cc.error(`${this.logTag}fail : ${args.url}`);
                            resolve(null);
                        }
                    });
                }
