...
export default class EventComponent extends cc.Component {
     ...
     */
    registerEvent(manCmd: number, subCmd: number, func: (data: any) => void, handleType?: any, isQueue?: boolean);
    /**
     ...
     * @param isQueue 是否加入队列
     */
    addEvent(manCmd: number, subCmd: number, func: (data: any) => void, handleType?: any, isQueue?: boolean);
    /**
     * 注册事件 ，在onLoad中注册，在onDestroy自动移除
     * @param eventName 
     * @param func 
     ...
     * @param subCmd 子cmd
     */
    removeEvent(manCmd: number, subCmd: number);
    /**
     * @description 删除普通事件
     * @param eventName 事件名
     */
