...
import StoneAgePageViewIndicator from "./StoneAgePageViewIndicator";
 ...
 */
export default class StoneAgePageViewEx extends StoneAgeComponentBase {
    ...
    @property({ type: StoneAgePageViewIndicator, tooltip: '分页脚标' })
    private indicator: StoneAgePageViewIndicator = null;//分页脚标
    //渲染事件（渲染器）
    @property({
    ...
    private createItem(index: number, isBehind: boolean = true): void {
        ...
        StoneAgeLogManager.instance.log('创建子项:', index);
    ...
    private checkLeftNeddRecover(): void {
        ...
        if (this.content.x + leftNodeInfo.node.x < -this.perDistance) {
            ...
            StoneAgeLogManager.instance.log('回收子项:', leftNodeInfo.index);
        }
    }
    ...
    private checkRightNeddRecover(): void {
        ...
        if (this.content.x + rightNodeInfo.node.x > this.perDistance) {
            ...
            StoneAgeLogManager.instance.log('回收子项:', rightNodeInfo.index);
    ...
    public jumpToPage(page: number): void {
        if (this.loop) {
            StoneAgeLogManager.instance.log('循环分页不支持跳转到指定的页数');
            return;
        }
        if (page < 0 || page >= this.totalPage) {
            StoneAgeLogManager.instance.log('跳转的页数请在0-' + (this.totalPage - 1));
            return;
        }
