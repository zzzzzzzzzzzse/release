import { BtnExStatus, ButtonMoveStatus } from "../../Const/StoneAgeCommonDefine";
import StoneAgeLogManager from "../../Manager/StoneAgeLogManager";
import { StoneAgeSlotAudioManager } from "../../Manager/StoneAgeSlotAudioManager";
...
@ccclass
@menu('自定义组件/StoneAgeButtonEx')
export default class StoneAgeButtonEx extends StoneAgeComponentBase {
    ...
    */
    private setLongPressTimer() {
        if (null != this._longPressCallback && !this.isLongPressRun) {
            this.longPressTimerId = StoneAgeTimerManager.instance.setInterval(() => {
                if (null == this._longPressCallback) {
                    return;
                }

                if (this.longPressMode == LongPressMode.CONTINUE) {
                    this.continueTimerId = StoneAgeTimerManager.instance.setInterval(() => {
            ...
            this.continueTimerId = StoneAgeTimerManager.instance.setInterval(this._longPressCallback, this.continueInterval);
        }
    }
    ...
    protected onClick() {
        if (this.cd) {
            this.cd = false;
            StoneAgeTimerManager.instance.setTimeOut(() => { this.cd = true; }, this.delay);
            if ("" != this.soundName) {
                StoneAgeSlotAudioManager.instance.playSound(this.aBName, this.soundName);
            }
    ...
    public setEnable() {
        ...
        for (let index = 0; index < this.grayLabelArr.length; index++) {
            const node = this.grayLabelArr[index];
            if (null != node) {
                let color = StoneAgeUtil.hexToRGB(parseInt(this.normalLabelColorArr[index]));
    ...
    public setDisable() {
        ...
        for (let index = 0; index < this.grayLabelArr.length; index++) {
            const node = this.grayLabelArr[index];
            if (null != node) {
                let color = StoneAgeUtil.hexToRGB(parseInt(this.grayLabelColorArr[index]));
                node.color = new cc.Color(color.r, color.g, color.b)
            }
        }
    }
     ...
     */
    private cleanTimer() {
        StoneAgeTimerManager.instance.clearInterval(this.continueTimerId);
        StoneAgeTimerManager.instance.clearInterval(this.longPressTimerId);
    }
    ...
    private scaleTouch() {
        ...
        if (1 == this.zoomScale) {
            StoneAgeLogManager.instance.err("[ButtonEx] 除数不能为0，无法缩放");
            return;
        }
    ...
    private normalPropertyChange() {
        for (let index = 0; index < this.normalOrDownPropertyChange.length; index++) {
            ...
            if (null == comp) {
                StoneAgeLogManager.instance.warn("[ButtonEx] normalPropertyChange ", arr[0], " is null");
                continue;
            }
            switch (arr[1]) {
                case "color":
                    let rgb = StoneAgeUtil.hexToRGB(parseInt(arr[2]));
    ...
    private downPropertyChange() {
        for (let index = 0; index < this.normalOrDownPropertyChange.length; index++) {
            ...
            if (null == comp) {
                StoneAgeLogManager.instance.warn("[ButtonEx] downPropertyChange ", arr[0], " is null");
                continue;
            }
            switch (arr[1]) {
                case "color":
                    let rgb = StoneAgeUtil.hexToRGB(parseInt(arr[3]));
                    comp[arr[1]] = new cc.Color(rgb.r, rgb.g, rgb.b);
                    break;
                default:
                    comp[arr[1]] = arr[3];
