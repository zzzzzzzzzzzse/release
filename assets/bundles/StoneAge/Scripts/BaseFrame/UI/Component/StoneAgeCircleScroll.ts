import StoneAgeCircleScrollItem from "./StoneAgeCircleScrollItem";
...
@ccclass
@menu('自定义组件/StoneAgeCircleScroll')
export default class StoneAgeCircleScroll extends cc.Component {
    ...
    private createItem(index: number, isRight: boolean = true): void {
        ...
        if (!nodeInfo) {
            ...
            const node = cc.instantiate(this.itemPrefab);
            const circleScrollItem = node.getComponent(StoneAgeCircleScrollItem);
