...
export default class StoneAgeHttpManager {
    private static _instance: StoneAgeHttpManager = null;

    /**
     * 获取单例
     */
    public static get instance(): StoneAgeHttpManager {
        if (!this._instance) {
            this._instance = new StoneAgeHttpManager();
        }
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState == 4) {
                ...
                if (xhr.status >= 200 && xhr.status < 400) {
                    ...
                    StoneAgeLogManager.instance.err(url + '请求失败, 内容为:', xhr.responseText);
    ...
    public sendRequestPost(url: string, data: any, callback?: (response: string) => void, timeOut: number = 3000) {
        ...
        setTimeout(() => {
            if (isTimeOut) {
                StoneAgeLogManager.instance.err(url + '请求超时失败');
                xhr.abort();
            }
        }, xhr.timeout);
        return xhr;
