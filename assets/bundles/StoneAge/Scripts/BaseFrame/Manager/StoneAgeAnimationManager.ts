 ...
 * 动画管理
 */
export default class StoneAgeAnimationManager {
    private backPlayMap: { [key: number]: { animation: cc.Animation } } = {};//需要在切换到后台的时候依旧播放的动画数组
    private static _instance: StoneAgeAnimationManager;
    private id: number = 0;//每个动画的唯一id
    public static get instance() {
        if (StoneAgeAnimationManager._instance == null) {
            StoneAgeAnimationManager._instance = new StoneAgeAnimationManager();
        }
        return StoneAgeAnimationManager._instance;
    }

    public constructor() {
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.QUIE_GAME_SHOW, (interval: number) => {
    ...
    public async playAnimation(data: {
        ...
        let res: cc.Prefab = await StoneAgeSlotResManager.instance.loadByBundle(data.animBundle, data.animPath);
