...
@ccclass
export default class StoneAgeSlotEventManager {
    private static _instance: StoneAgeSlotEventManager
    public static get instance() {
        if (StoneAgeSlotEventManager._instance == null) {
            StoneAgeSlotEventManager._instance = new StoneAgeSlotEventManager();
        }
        return StoneAgeSlotEventManager._instance;
    }
    ...
    public emit(type: string, data?: any) {
        let eventArr = this.eventMap[type];
        if (null == eventArr) {
            StoneAgeLogManager.instance.err(type, "event is null");
            return;
        }
        for (let index = 0; index < eventArr.length; index++) {
