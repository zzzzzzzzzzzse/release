import StoneAgeHttpManager from "./StoneAgeHttpManager";
import StoneAgeLogManager from "./StoneAgeLogManager";

export default class StoneAgeFileManager {
    private static _instance: StoneAgeFileManager = null;
     ...
     * 获取单例
     */
    public static get instance(): StoneAgeFileManager {
        if (!this._instance) {
            this._instance = new StoneAgeFileManager();
        }
    ...
    public downloadBinaryFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        if (count > this.maxCount) {
            ...
            StoneAgeLogManager.instance.log('达到最大下载数量，进入排队队列');
            return;
        }
    ...
    public downloadTxtFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        this.requestList[url] = callback;
        const download = url => {
            StoneAgeHttpManager.instance.sendRequestGet(url, response => {
                const callback = this.requestList[url];
    ...
    private downloadWaitList(): void {
        ...
        StoneAgeLogManager.instance.log('从队列中下载文件:', info.url);
        if (info.isBinary) {
