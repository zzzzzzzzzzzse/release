 ...
 * 计时器
 */
export default class StoneAgeTimerManager {
    private static _instance: StoneAgeTimerManager
    public static get instance() {
        if (StoneAgeTimerManager._instance == null) {
            StoneAgeTimerManager._instance = new StoneAgeTimerManager();
        }
        return StoneAgeTimerManager._instance;
    }
    ...
    public setInterval(callback: Function, interval: number = 0, repeat: number = null): number {
        let count = 0;
        if (repeat < 0) {
            StoneAgeLogManager.instance.err("Repeat must be equal or greater than 0");
            return;
        }
        if (null == repeat) {
            repeat = -1;
