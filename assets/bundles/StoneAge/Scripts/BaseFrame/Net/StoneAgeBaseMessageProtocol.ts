...
import StoneAgeProtocolBase from "./StoneAgeProtocolBase";

export default class StoneAgeBaseMessageProtocol extends StoneAgeProtocolBase {
    protected pbPackageName: string = "com.islet.game.proto";//当前协议包名
    protected pbName: string = "PB/BaseMessage";//pb文件名，不带扩展名

    private static _instance: StoneAgeBaseMessageProtocol = null;
     ...
     * 获取单例
     */
    public static get instance(): StoneAgeBaseMessageProtocol {
        if (!this._instance) {
            this._instance = new StoneAgeBaseMessageProtocol();
        }
