import { BundleType, CommonTopStatus, SoundsType, SpinBtnStatus, TurntableStatus } from "./BaseFrame/Const/StoneAgeCommonDefine";
...
import { StoneAgeActionType, StoneAgeGameType, StoneAgeSoundsType, StoneAgeWinType } from "./StoneAgeDefine";
...
@ccclass
export default class StoneAgeTurnTable extends StoneAgeComponentBase {
    @property(cc.Node)
    ...
    private addEvent() {
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.SLOTS_CHANGE_ICON_IMAGE, (data: { id: number, isDim: boolean }) => {
            this.changeIconImg(data);
        }, this);

        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.UPDATE_CIRCLE, (data: { index: number, times: number }) => {
            this.updateCircle(data);
        }, this);

        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.WIN_DIALOG_CLOSE_CALLBACK, () => {
        ...
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.WIN_DIALOG_OPEN_CALLBACK, () => {
            this.bigWinOpenCallback();
        }, this);
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.CHECK_SERVER_SLOT_DATA, this.checkServerBetData, this);
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.WHEEL_COLLIDE_BOTTOM, this.collideBottom, this);
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.SLOTS_SPEED_UP_START, this.turnStageSpeedUp, this);
    }
    ...
    private collideBottom(event) {
        StoneAgeSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.END_ROLL);
    }
    ...
    private drawPayAlllineTwo(callback: Function) {
        let tsStoneAgeView: StoneAgeView = <StoneAgeView>this.viewRootNode.getComponent("StoneAgeView");
        if (null != StoneAgeProxy.instance.gameOverData && StoneAgeProxy.instance.gameOverData.totalPay > 0 && StoneAgeProxy.instance.gameOverData.winType <= StoneAgeWinType.WIN) {
            this.tsSlotsBottomBtn.setWinNum({ num: StoneAgeProxy.instance.gameOverData.totalPay, isChange: true });
            this.tsSlotsBottomBtn.setWinTitle(false);
        }
        tsStoneAgeView.updateSelfMoney();
        StoneAgeLogManager.instance.info("开始绘制所有赔付线");
     ...
     */
    public checkIsClick() {
        let tsCommonTop: StoneAgeCommonTop = <StoneAgeCommonTop>this.tsStoneAgeView.topNode.getComponent(StoneAgeCommonTop);
        let isJump = tsCommonTop.checkShowLevelRoad(false);
        if (StoneAgeProxy.instance.gameOverData.free == 0
            && !StoneAgeProxy.instance.gameOverData.freeGameSettle
            && !isJump) {
            this.tsStoneAgeView.isCanClick();
        }
    }
    ...
    */
    public checkIsClickFront() {
        let tsCommonTop: StoneAgeCommonTop = <StoneAgeCommonTop>this.tsStoneAgeView.topNode.getComponent(StoneAgeCommonTop);
        let isJump = tsCommonTop.checkShowLevelRoad(false);
        if (null != StoneAgeProxy.instance.gameOverData
            && StoneAgeProxy.instance.gameOverData.free == 0
            && !StoneAgeProxy.instance.gameOverData.freeGameSettle
            && !isJump
            && StoneAgeProxy.instance.gameOverData.winType <= StoneAgeWinType.WIN
        ) {
            this.tsStoneAgeView.isCanClick();
        }
    }
    ...
    private playEveryOneSound(id: number) {
        let path = StoneAgeSoundsType.EVERY_LINE;
        // switch (id) {
        //     case StoneAgeIconType.FREE:
        //         path = StoneAgeSoundsType.FREE;
        //         break;
        // }
        StoneAgeSlotAudioManager.instance.playSound(BundleType.STONE_AGE, path);
    }
    ...
    public drawPayEveryOneLine(callbackYes: Function) {
        if (this.tsSlotsBottomBtn.getIsAutoBet()
            || StoneAgeProxy.instance.gameOverData.gameType == StoneAgeGameType.FREE
            || StoneAgeProxy.instance.freeSpinCount != 0) {
            if (callbackYes) {
                callbackYes();
            }
        }
            ...
            this.tsStoneAgeView.isCanClick();
            StoneAgeLogManager.instance.info("开始分别绘制每条赔付线");
            ...
            if (0 == linesDataArr.length) {
                StoneAgeLogManager.instance.info("没有赔付线");
                if (callbackYes) {
                    callbackYes();
                }
            }
    ...
    private drawLine(linesDataArr: Array<{ linePoints: Array<cc.Vec2>, colorHex: number }>, opacity: number) {
        ...
        for (let index = 0; index < linesDataArr.length; index++) {
            ...
            let colorRGB: { r: number, g: number, b: number } = StoneAgeUtil.hexToRGB(lineOne.colorHex);
    ...
    private drawRect(rectArr: Array<{ colorHex: number, node: cc.Node }>, opacity: number) {
        ...
        for (let index = 0; index < rectArr.length; index++) {
            ...
            let tsLuckyCloverItem: StoneAgeItem = <StoneAgeItem>rectOne.node.getComponent("StoneAgeItem");
            ...
            let colorRGB: { r: number, g: number, b: number } = StoneAgeUtil.hexToRGB(rectOne.colorHex);
            graphics.strokeColor = cc.color(colorRGB.r, colorRGB.g, colorRGB.b, opacity);
            graphics.rect(
                vec2.x - 76,
                vec2.y - 65.5,
    ...
    public cleanResultAnim() {
        StoneAgeLogManager.instance.info("清空所有动画");
        cc.Tween.stopAllByTarget(this.tweenObj);
        cc.Tween.stopAllByTarget(this.particleNode);
        cc.Tween.stopAllByTarget(this.particleOneNode);
        cc.Tween.stopAllByTarget(this.particleTwoNode);
        ...
        //讲隐藏的图片透明度还原
        this.tsStoneAgeView.setOpacityFull();
        this.tsSlotsBottomBtn.setNotOperationAnim();
        let tsStoneAgeView: StoneAgeView = <StoneAgeView>this.viewRootNode.getComponent("StoneAgeView");
        tsStoneAgeView.tsStoneAgeOperate.updateBet(false, false);
    }
    ...
    public checkServerData(callbackYes: Function, callbackNo: Function) {
        ...
        if (StoneAgeProxy.instance.gameOverData == null) {
            StoneAgeLogManager.instance.log("未收到服务器数据");
            if (StoneAgeProxy.instance.freeSpinCount == 0) {
                let tsStoneAgeView: StoneAgeView = <StoneAgeView>this.viewRootNode.getComponent("StoneAgeView");
                this.tsSlotsBottomBtn.setIsAutoBet(false);
                if (!this.tsSlotsBottomBtn.getIsAutoBet()) {
                    tsStoneAgeView.tsStoneAgeOperate.setButtonCanClick();
                }
            ...
            let tsStoneAgeView: StoneAgeView = <StoneAgeView>this.viewRootNode.getComponent("StoneAgeView");
            tsStoneAgeView.restoreSelfMoney();
            StoneAgeProxy.instance.initAddCircleStartIndex();
            if (callbackNo) {
                callbackNo();
            }
        }
    ...
    private playOneWinAnim(node: cc.Node) {
        let tsLuckyCloverItem: StoneAgeItem = <StoneAgeItem>node.getComponent("StoneAgeItem");
    ...
    public isAutoSpin() {
        if (this.tsSlotsBottomBtn.getIsAutoBet()) {
            ...
            this.tsSlotsBottomBtn.setSpinStatus(SpinBtnStatus.SPIN);
            if (StoneAgeProxy.instance.gameOverData != null && !StoneAgeProxy.instance.gameOverData.freeGameSettle) {
                StoneAgeLogManager.instance.info("开始分别画线");
                // this.drawPayEveryOneLine();
            }
            else {
                StoneAgeLogManager.instance.info("进入清理阶段");
    ...
    private handlePayLinesData(): Array<{
        ...
        // let linesArr = StoneAgeProxy.instance.getPayLinesData();
        ...
        //         let combo = StoneAgeProxy.instance.getComboByLinesId(line.id);
        //         let num = combo;
        ...
        //         }
        //         linePosArr.push({ linePoints: _linePosArr, colorHex: line.color, rectArr: _rectArr, targetId: StoneAgeProxy.instance.getTargetIdByLinesId(line.id) });
        //     }
        // }
        // return linePosArr;
        return null;
    ...
    public scrollStopOne(index: number) {
        ...
        let tsStoneAgeScroll: StoneAgeScroll = <StoneAgeScroll>this.listScroll[index].getComponent("StoneAgeScroll");
        //停止当前加圈动画
        tsStoneAgeScroll.stopAddCircleSpine();
        if (index >= StoneAgeProxy.instance.getAddCircleStartIndex() - 1 &&
            -1 != StoneAgeProxy.instance.getAddCircleStartIndex() &&
            index != this.scrollNodeArr.length - 1 &&
            StoneAgeProxy.instance.gameOverData != null) {
            StoneAgeSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.EXTEND_ROLL);
            let tsNextLuckyCloverScroll: StoneAgeScroll = <StoneAgeScroll>this.listScroll[index + 1].getComponent("StoneAgeScroll");
            if (null != tsNextLuckyCloverScroll) {
                //播放下一个轴的加圈动画
                tsNextLuckyCloverScroll.playAddCircleSpine();
            }
        }
        let luckyCloverTimestamp = tsStoneAgeScroll.playFreeAnim();
        ...
        if (this.currScrollStopCount == this.scrollNodeArr.length) {
            StoneAgeSlotEventManager.instance.emit(StoneAgeEventDefine.SLOTS_TURN_ALL_END);
            ...
            if (currTimestamp >= this.scrollAnimTimestamp) {
                ...
                //等待免费转动画自然播完再结算
                this.delayResultTimerId = this.setTimeOut(() => { StoneAgeAcitonManager.instance.addAction({ type: StoneAgeActionType.RESULT }); }, (this.scrollAnimTimestamp - currTimestamp) / 1000);
            }
    ...
    */
    ...
    public setWinNum() {
        let time: number;
        switch (StoneAgeProxy.instance.gameOverData.winType) {
            case StoneAgeWinType.BIG_WIN:
                time = 4.7;
                break;
            case StoneAgeWinType.MEGA_WIN:
                time = 4.7;
                break;
            case StoneAgeWinType.SUPER_WIN:
                time = 4.7;
                break;
            case StoneAgeWinType.HUGE_WIN:
                time = 4.7;
                break;
        }
        this.tsSlotsBottomBtn.setWinNum({ num: StoneAgeProxy.instance.gameOverData.totalPay, isChange: true, isBigWin: true, bigWinTime: time });
        this.tsSlotsBottomBtn.setWinTitle(false);
        let tsStoneAgeView: StoneAgeView = <StoneAgeView>this.viewRootNode.getComponent("StoneAgeView");
        tsStoneAgeView.updateSelfMoney();
    }
    ...
    */
    ...
    public playAllItemAnim(tableId: number) {
        for (let index = 0; index < this.scrollNodeArr.length; index++) {
            ...
            for (let i = 0; i < scrollNode.children.length; i++) {
                ...
                let tsLuckyCloverItem: StoneAgeItem = <StoneAgeItem>item.getComponent("StoneAgeItem");
    ...
    public checkLevelRoad(callback: Function): boolean {
        let tsStoneAgeView: StoneAgeView = <StoneAgeView>this.viewRootNode.getComponent("StoneAgeView");
        let tsCommonTop: StoneAgeCommonTop = <StoneAgeCommonTop>tsStoneAgeView.topNode.getComponent(StoneAgeCommonTop);
