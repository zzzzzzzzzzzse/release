...
import ProgressBarEx, { ProgressBarEvent } from "../BaseFrame/UI/Component/StoneAgeProgressBarEx";
...
@ccclass
export default class StoneAgeCommonTop extends StoneAgeComponentBase {
    @property(ButtonEx)
    ...
    private goldLabel: StoneAgeLabelEx = null;//金币
    @property(StoneAgeLabelEx)
    private levelLabel: StoneAgeLabelEx = null;//等级
    ...
    private addHandler(): void {
        ...
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.CLOSE_MENU_DIALOG, this.onMenuClose, this);
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.USERINFO_CHANGED, this.updateAvatar, this);
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.UPDATE_PLAYER_GOLD, this.playGoldChanged, this);
        StoneAgeSlotEventManager.instance.on(StoneAgeEventDefine.COMMON_TOP_STATUS_CHANGE, (status: CommonTopStatus) => {
    ...
    public updatePlayerGold(isDigit: boolean = true, descGold: number = -1): void {
        ...
        this.goldLabel.label.string = StoneAgeUtil.formateCoinStr(gold);
    }
    ...
    private onAvatar(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            StoneAgeTipsManager.instance.showTips(this.tipKey);
        }
        else {
            StoneAgeDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/InformationDialog');
        }
    }
    ...
    private onShop(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            StoneAgeViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/ShopView', null, this.isHall, ViewCloseAnimType.ONE, ViewOpenAnimType.ONE);
        }
    }
    ...
    private onVip(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            StoneAgeDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/PrimeDialog');
        }
    ...
    private onSaveMoney(): void {
        ...
        const unlock = StoneAgeUtil.checkFunctionOpen(FunctionKey.BANK_PIG);
        if (!unlock) {
            return;
        }
        StoneAgeViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/BankView', null, this.isHall);
    }
    ...
    private onMenu(): void {
        this.showMenu = true;
        StoneAgeDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/MenuDialog', this.tipKey);
