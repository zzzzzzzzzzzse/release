...
import { BundleType, CommonTopStatus, GuideGroup, MaxStatus, SlotsViewScaleStatus, SoundsType, SpinAnimStatus, SpinBtnStatus } from "../BaseFrame/Const/StoneAgeCommonDefine";
...
import StoneAgeCommonTop from "./StoneAgeCommonTop";
...
@ccclass
export default class StoneAgeSlotsBottomBtn extends StoneAgeComponentBase {
    @property(ButtonEx)
    ...
    private updateBetGold(): void {
        this.betLabelNode.getComponent(cc.Label).string = '' + StoneAgeProxy.instance.betGold;
    }

    onDisable() {
        StoneAgeLogManager.instance.log('清理SlotsBottomBtn的所有延迟调用');
        this.clearAllEvent();
    ...
    */
    ...
    public cancleNotOperationAnim() {
        StoneAgeLogManager.instance.log("[SlotsBottomBtn] cancleNotOperationAnim");
    ...
    private onClickAddBet() {
        ...
        if (StoneAgeProxy.instance.betGold >= 10000) {
            StoneAgeProxy.instance.betGold = 1000;
            this.updateBetGold();
            return;
        }
        StoneAgeProxy.instance.betGold += 1000;
    ...
    private onClickSubBet() {
        if (StoneAgeProxy.instance.betGold <= 1000) {
            StoneAgeProxy.instance.betGold = 10000;
            this.updateBetGold();
            return;
        }
        StoneAgeProxy.instance.betGold -= 1000;
    ...
    private onLongPressSubBet() {
        ...
        StoneAgeSlotEventManager.instance.emit(StoneAgeEventDefine.BET_GOLD_CHANGE, { index: this.currBetRangeIndex });
    ...
    public onClickSpin(testData?: string) {
        StoneAgeLogManager.instance.log("[SlotsBottomBtn] onClickSpin")
        if (!PlayerProxy.instance.roomId) {
            StoneAgeLogManager.instance.err("房间已销毁，不能投注")
            return;
        }
        if (PlayerProxy.instance.gold < StoneAgeProxy.instance.betGold) {
            return;
        }
        ...
        //先把钱扣了
        let tsCommonTop: StoneAgeCommonTop = <StoneAgeCommonTop>this.topNode.getComponent(StoneAgeCommonTop);
        PlayerProxy.instance.gold -= StoneAgeProxy.instance.betGold;
        tsCommonTop.updatePlayerGold(true, PlayerProxy.instance.gold);
        StoneAgeProxy.instance.randGameOverData();
    ...
    public freeSpin() {
        ...
        //播放音效
        StoneAgeSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.SPIN);
        if (this.getSpinStatus() == SpinBtnStatus.SPIN) {
            this.stopSpinSpineAnim();
        }
        this.cancleNotOperationAnim();
   ...
   */
    ...
    */
    private onLongPressSpinEndCallback() {
        StoneAgeLogManager.instance.log("[SlotsBottomBtn] onLongPressSpinEndCallback");
        // if (PlayerProxy.instance.gameId == parseInt(MessageType.CLASSIC777_MSG)) {
        //     const guideInfo = GuideProxy.instance.getCurrGuideInfo();
        //     if (guideInfo && guideInfo.group == GuideGroup.GUIDE_PLAY_GAME) {
        //         return;
        ...
        this.stopSpinSpineAnim();
        this.setNotOperationAnim();
        StoneAgeDeviceManager.instance.stopShake();
        //回调
    ...
    public playSpinSpineAnim(animStatus?: SpinAnimStatus, repeat: number = 0, callback?: Function) {
        ...
        if (animStatus == SpinAnimStatus.NULL) {
            StoneAgeLogManager.instance.log("stopBtnNull");
            this.stopSpinSpineAnim();
            return;
        }
    ...
    public setSpinStatus(status: SpinBtnStatus) {
        ...
        StoneAgeLogManager.instance.log("设置按钮状态为：", status);
    ...
    public async setMaxBetStatus(status: MaxStatus) {
        ...
        switch (status) {
            case MaxStatus.MAX:
                ...
                betRes = await StoneAgeSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt_max", cc.SpriteFrame);
            ...
            case MaxStatus.NOT_MAX:
                ...
                betRes = await StoneAgeSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt", cc.SpriteFrame);
                if (null != this.betTitleNode && this.betTitleNode.isValid) {
                    this.betTitleNode.getComponent(cc.Sprite).spriteFrame = betRes;
                }
                break;
    ...
    public setButtonCanNotClick() {
        StoneAgeLogManager.instance.log("[SlotsBottomBtn] setButtonCanNotClick");
    ...
    public setButtonCanClick() {
        StoneAgeLogManager.instance.log("[SlotsBottomBtn] setButtonCanClick");
    ...
    public setWinNum(data: { num: number, isChange: boolean, isBigWin?: boolean, bigWinTime?: number, initNum?: number }) {
        ...
        if (data.isChange) {
            ...
            this.winNumLabelNode.getComponent(StoneAgeLabelEx).digitEffect({ score: data.num, time: time, bit: 0, isScale: true, initNum: data.initNum });
        }
        else {
            this.winNumLabelNode.getComponent(StoneAgeLabelEx).stopDigitEffect();
