...
import { BundleType, ViewCloseAnimType, ViewLevel, ViewOpenAnimType } from "../Const/RichCommonDefine";
import RichViewBase from "../UI/View/RichViewBase";
...
@ccclass
export default class RichViewManager {
    ...
    private static _instance: RichViewManager
    public static get instance() {
        if (RichViewManager._instance == null) {
            RichViewManager._instance = new RichViewManager();
        }
        return RichViewManager._instance;
    }
    ...
    public openView(
        ...
        RichTimerManager.instance.clearInterval(this.delayReleaseIDMap[abundleName]);
    ...
    public back(
        ...
        for (let index = this.openListArr.length - 1; index >= 0; index--) {
            ...
            if (element.level == ViewLevel.LEVEL_ONE) {
                obj = element;
                RichLogManager.instance.log("openListArrBackOver：", this.openListArr);
                break;
            }
            else {
                ...
                RichLogManager.instance.log("openListArr：", this.openListArr);
            }
        }
        if (null == obj) {
            RichLogManager.instance.err("back view is null");
            return;
        }
        if (!isOpenLast) {
            return;
        }
        RichTimerManager.instance.clearInterval(this.delayReleaseIDMap[obj.abundleName]);
        this.opening(
            obj.abundleName,
            obj.path,
            obj.data,
    ...
    private handleView(
        ...
        let tsViewBase: RichViewBase = <RichViewBase>view.getComponent("ViewBase");
    ...
    private destroyLastView(isDestroy: boolean, closeAnimType: ViewCloseAnimType) {
        if (null != this.lastViewPath) {
            ...
            if (null != lastNode) {
                let tsOtherViewBase: RichViewBase = <RichViewBase>lastNode.getComponent("ViewBase");
                ...
                tsOtherViewBase.setClose(isDestroy, () => {
                    if (isGame && isDestroy) {
                        this.delayReleaseIDMap[_abundleName] = RichTimerManager.instance.setInterval(() => {
                            RichSlotResManager.instance.releaseBundle(_abundleName);
                            RichLogManager.instance.info("释放成功:", this.delayReleaseIDMap[_abundleName], " name:", _abundleName);
                        }, 0, 0);
                        RichLogManager.instance.info("开始释放：", _abundleName, "计时器：", this.delayReleaseIDMap[_abundleName]);
                    }
                }, closeAnimType);
    ...
    public returnHall() {
        ...
        this.clean();
        RichSlotResManager.instance.cleanGameBundle();
    ...
    public returnLogin() {
        ...
        this.clean();
        RichSlotResManager.instance.cleanAllBundle();
    ...
    private clean() {
        for (const key in this.viewMap) {
            ...
            if (null != element.node) {
                let tsOtherViewBase: RichViewBase = <RichViewBase>element.node.getComponent("ViewBase");
