import RichHttpManager from "./RichHttpManager";
import RichLogManager from "./RichLogManager";

export default class RichFileManager {
    private static _instance: RichFileManager = null;
     ...
     * 获取单例
     */
    public static get instance(): RichFileManager {
        if (!this._instance) {
            this._instance = new RichFileManager();
        }
    ...
    public downloadBinaryFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        if (count > this.maxCount) {
            ...
            RichLogManager.instance.log('达到最大下载数量，进入排队队列');
            return;
        }
    ...
    public downloadTxtFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        this.requestList[url] = callback;
        const download = url => {
            RichHttpManager.instance.sendRequestGet(url, response => {
                const callback = this.requestList[url];
    ...
    private downloadWaitList(): void {
        ...
        RichLogManager.instance.log('从队列中下载文件:', info.url);
        if (info.isBinary) {
