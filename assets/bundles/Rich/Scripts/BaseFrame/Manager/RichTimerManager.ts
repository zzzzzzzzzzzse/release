 ...
 * 计时器
 */
export default class RichTimerManager {
    private static _instance: RichTimerManager
    public static get instance() {
        if (RichTimerManager._instance == null) {
            RichTimerManager._instance = new RichTimerManager();
        }
        return RichTimerManager._instance;
    }
    ...
    public setInterval(callback: Function, interval: number = 0, repeat: number = null): number {
        let count = 0;
        if (repeat < 0) {
            RichLogManager.instance.err("Repeat must be equal or greater than 0");
            return;
        }
        if (null == repeat) {
            repeat = -1;
