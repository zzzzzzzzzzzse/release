...
@ccclass
export default class RichSlotEventManager {
    private static _instance: RichSlotEventManager
    public static get instance() {
        if (RichSlotEventManager._instance == null) {
            RichSlotEventManager._instance = new RichSlotEventManager();
        }
        return RichSlotEventManager._instance;
    }
    ...
    public emit(type: string, data?: any) {
        let eventArr = this.eventMap[type];
        if (null == eventArr) {
            RichLogManager.instance.err(type, "event is null");
            return;
        }
        for (let index = 0; index < eventArr.length; index++) {
