...
export default class RichSlotResManager {
    private static _instance: RichSlotResManager
    public static get instance() {
        if (RichSlotResManager._instance == null) {
            RichSlotResManager._instance = new RichSlotResManager();
        }
        return RichSlotResManager._instance;
    }
    ...
    public async loadBundle(aBName: string): Promise<cc.AssetManager.Bundle> {
        let startTime: number = RichUtil.getTimeStamp();
        let endTime: number;
        return new Promise<any>((resolve, reject) => {
            cc.assetManager.loadBundle(aBName, (err, bundle) => {
                if (err) {
                    RichLogManager.instance.log("loadBundleFail:", aBName);
                    reject(err.message || err);
                }
                else {
                    endTime = RichUtil.getTimeStamp();
                    RichLogManager.instance.log("loadBundleSucc:", aBName, " costTime:", endTime - startTime, "ms");
                    resolve(bundle);
                }
            });
        })
    ...
    public async loadByBundle(aBName: string, path: string, type?: typeof cc.Asset, onProgress?: (finish: number, total: number) => void): Promise<any> {
        ...
        type ||= cc.Asset;
        if (null == abundle) {
            RichLogManager.instance.err("abundle is null:", aBName);
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.load(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = RichUtil.getTimeStamp();
                        RichLogManager.instance.log("loadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        RichLogManager.instance.err(err)
                        reject(err.message || err);
                    }
                }
            );
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.loadDir(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = RichUtil.getTimeStamp();
                        RichLogManager.instance.log("loadDirByBundleSucc:", aBName, "->", dir, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        reject(err.message || err);
    ...
    public releaseBundle(aBName: string) {
        ...
        if (null != bundle) {
            ...
            RichLogManager.instance.log('释放bundle：', aBName);
            ...
            abundle.preloadDir(
                ...
                (err, asset) => {
                    if (null != err) {
                        ...
                        endTime = RichUtil.getTimeStamp();
                        RichLogManager.instance.log("preloadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                }
            );
