...
import RichProtocolBase from "./RichProtocolBase";

export default class RichBaseMessageProtocol extends RichProtocolBase {
    protected pbPackageName: string = "com.islet.game.proto";//当前协议包名
    protected pbName: string = "PB/BaseMessage";//pb文件名，不带扩展名

    private static _instance: RichBaseMessageProtocol = null;
     ...
     * 获取单例
     */
    public static get instance(): RichBaseMessageProtocol {
        if (!this._instance) {
            this._instance = new RichBaseMessageProtocol();
        }
