...
export default abstract class RichProtocolBase {
    ...
    public async encode(data: any, msgName: string, needLog: boolean = true): Promise<any> {
        ...
        if (!this.pb || !this.pb[msgName]) {
            RichLogManager.instance.log(msgName + '协议未定义');
        }
        ...
        needLog && RichLogManager.instance.log('send===============' + msgName + '===============', data);
    ...
    public async decode(buffer: any, msgName: string, needLog: boolean = true) {
        ...
        needLog && RichLogManager.instance.log('recv===============' + msgName + '===============', data);
        return data;
    }
