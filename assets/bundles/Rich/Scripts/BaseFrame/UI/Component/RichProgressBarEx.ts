...
@ccclass
@menu('自定义组件/RichProgressBarEx')
export default class RichProgressBarEx extends RichComponentBase {
    @property({
    ...
    public setProgress(data: { current: number, total: number, isTween?: boolean, time?: number, overMaxIsClean?: boolean, easing?: string }) {
        ...
        data.current = RichUtil.getFloatToFixed(data.current, 0, false);
    ...
    public setProgressPercent(data: { percent: number, isTween?: boolean, time?: number, overMaxIsClean?: boolean, easing?: string }) {
        ...
        let current = RichUtil.getFloatToFixed(data.percent * 100, 0, false);
        let total = 100;
    ...
    private handle(width: number, curr: number, total: number) {
        ...
        switch (this.labelMode) {
            case LabelMode.PERCENT:
                this.txtLabel.string = RichUtil.getFloatToFixed(curr / total * 100, 0, false) + "%";
                break;
            case LabelMode.CURRENT_OR_TOTAL:
                this.txtLabel.string = RichUtil.getFloatToFixed(curr, 0, false) + "/" + total;
                break;
        }
    }
