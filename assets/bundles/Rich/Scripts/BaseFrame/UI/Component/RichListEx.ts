...
@ccclass
@disallowMultiple()
@menu('自定义组件/RichListEx')
@requireComponent(cc.ScrollView)
//脚本生命周期回调的执行优先级。小于 0 的脚本将优先执行，大于 0 的脚本将最后执行。该优先级只对 onLoad, onEnable, start, update 和 lateUpdate 有效，对 onDisable 和 onDestroy 无效。
@executionOrder(-5000)
export default class RichListEx extends cc.Component {
    //缓冲区域
    @property
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            case SelectedType.SINGLE: {
                ...
                t._selectedId = val;
                if (item) {
                    listItem = item.getComponent(RichListItem);
                    listItem.selected = true;
                }
                if (t._lastSelectedId >= 0 && t._lastSelectedId != t._selectedId) {
                    let lastItem: any = t.getItemByListId(t._lastSelectedId);
                    if (lastItem) {
                        lastItem.getComponent(RichListItem).selected = false;
                    }
                }
                if (t.selectedEvent) {
            ...
            case SelectedType.MULT: {
                ...
                if (!item)
                    return;
                let listItem = item.getComponent(RichListItem);
                if (t._selectedId >= 0)
                    t._lastSelectedId = t._selectedId;
                t._selectedId = val;
    ...
    setTemplateItem(item: any) {
        ...
        let com = item.getComponent(RichListItem);
        let remove = false;
        if (!com)
            remove = true;
        // if (com) {
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            ...
            let listItem: RichListItem = item.getComponent(RichListItem);
    ...
    _updateListItem(listItem: RichListItem) {
        if (!listItem)
            return;
