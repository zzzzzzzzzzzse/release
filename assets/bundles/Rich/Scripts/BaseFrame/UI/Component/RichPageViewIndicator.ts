...
import RichPageViewIndicatorItem from "./RichPageViewIndicatorItem";
...
export default class RichPageViewIndicator extends RichComponentBase {
    //模板类型
    ...
    public addItem(): void {
        RichLogManager.instance.log('增加一个页标');
    ...
    public setPages(pages: number): void {
        RichLogManager.instance.log('清空页标');
    ...
    public setCurrentPage(index: number = 0): void {
        ...
        for (let i = 0; i < this.itemList.length; i++) {
            ...
            const item = node.getComponent(RichPageViewIndicatorItem);
