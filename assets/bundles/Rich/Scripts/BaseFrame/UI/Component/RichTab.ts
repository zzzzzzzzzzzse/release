...
import RichListEx from "./RichListEx";
import RichTabItem from "./RichTabItem";

const { ccclass, property, menu } = cc._decorator;

@ccclass
@menu('自定义组件/Tab/RichTab')
export default class RichTab extends RichComponentBase {
    @property({
        type: [RichTabItem],
        visible() {
            return !this.virtual;
    ...
    private virtualSelectChanged(): void {
        ...
        for (let i = 0; i < itemList.length; i++) {
            const tabItem = itemList[i].getComponent(RichTabItem);
        ...
        if (selectNode) {
            const selectItem = selectNode.getComponent(RichTabItem);
    ...
    private itemChanged(node: cc.Node, index: number): void {
        const tabItem = node.getComponent(RichTabItem);
        if (!tabItem) {
            RichLogManager.instance.warn(node.name + '必现继承TabItem');
            return;
        }
