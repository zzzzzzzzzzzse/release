...
import RichPageViewIndicator from "./RichPageViewIndicator";
 ...
 */
export default class RichPageViewEx extends RichComponentBase {
    ...
    @property({ type: RichPageViewIndicator, tooltip: '分页脚标' })
    private indicator: RichPageViewIndicator = null;//分页脚标
    //渲染事件（渲染器）
    @property({
    ...
    private createItem(index: number, isBehind: boolean = true): void {
        ...
        RichLogManager.instance.log('创建子项:', index);
    ...
    private checkLeftNeddRecover(): void {
        ...
        if (this.content.x + leftNodeInfo.node.x < -this.perDistance) {
            ...
            RichLogManager.instance.log('回收子项:', leftNodeInfo.index);
        }
    }
    ...
    private checkRightNeddRecover(): void {
        ...
        if (this.content.x + rightNodeInfo.node.x > this.perDistance) {
            ...
            RichLogManager.instance.log('回收子项:', rightNodeInfo.index);
    ...
    public jumpToPage(page: number): void {
        if (this.loop) {
            RichLogManager.instance.log('循环分页不支持跳转到指定的页数');
            return;
        }
        if (page < 0 || page >= this.totalPage) {
            RichLogManager.instance.log('跳转的页数请在0-' + (this.totalPage - 1));
            return;
        }
