import { SlotsStage } from "../../Const/RichCommonDefine";
...
@ccclass
export default class RichSlotWheelBase extends cc.Component {
    ...
    private onStop(): void {
        ...
        RichSlotEventManager.instance.emit(RichEventDefine.QUIE_SLOTS_ONE_STOP, { index: this.index, isCopy: this.isCopy });
    }
    ...
    public calcDistance(): void {
        if (this.time >= this.t0 + this.t1 + this.t2 + this.t3) {
            if (this.stage != SlotsStage.BOUNCE) {
                ...
                RichSlotEventManager.instance.emit(RichEventDefine.SLOTS_TURN_STAGE, { id: this.index, stage: this.stage });
                RichSlotEventManager.instance.emit(RichEventDefine.SLOTS_BOUNCE_START, { index: this.index })
            }
            ...
            if (this.time + this.uniformPhaseuUnitTs >= this.t0 + this.t1 + this.t2 && 0 == this.index) {
                RichLogManager.instance.info("检查：", this.curDistance, " index:", this.index);
                RichSlotEventManager.instance.emit(RichEventDefine.CHECK_SERVER_SLOT_DATA);
            }
        }
        else if (this.time >= this.t0) {
            if (this.stage != SlotsStage.SPEED_UP) {
                this.stage = SlotsStage.SPEED_UP;
                RichSlotEventManager.instance.emit(RichEventDefine.SLOTS_SPEED_UP_START, { index: this.index })
            }
            const t0 = this.t0;
        ...
        if (!this.isChangeDimImage && this.curDistance >= this.perDistance) {
            RichLogManager.instance.info("换图模糊：", this.index)
        ...
        if (!this.isChangeClearnessImage && this.curDistance >= this.distance - this.perDistance) {
            RichLogManager.instance.info("换图清晰：", this.index)
            this.isChangeClearnessImage = true;
            RichSlotEventManager.instance.emit(RichEventDefine.SLOTS_CHANGE_ICON_IMAGE, { isDim: false, id: this.index });
        }
        if (!this.isCollideBottom && this.curDistance >= (this.distance + this.debugX4)) {
            RichLogManager.instance.info("撞底：", this.index)
            this.isCollideBottom = true;
            RichSlotEventManager.instance.emit(RichEventDefine.WHEEL_COLLIDE_BOTTOM, { id: this.index });
        }
