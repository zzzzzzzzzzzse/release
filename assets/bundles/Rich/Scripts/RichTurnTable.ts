import { BundleType, CommonTopStatus, SoundsType, SpinBtnStatus, TurntableStatus } from "./BaseFrame/Const/RichCommonDefine";
...
import { RichActionType, RichGameType, RichSoundsType, RichWinType } from "./RichDefine";
...
@ccclass
export default class RichTurnTable extends RichComponentBase {
    @property(cc.Node)
    ...
    private tsRichView: RichView = null;
    onLoad() {
        this.tsRichView = this.node.getComponent("RichView");
    }
    ...
    private addEvent() {
        RichSlotEventManager.instance.on(RichEventDefine.SLOTS_CHANGE_ICON_IMAGE, (data: { id: number, isDim: boolean }) => {
            this.changeIconImg(data);
        }, this);

        RichSlotEventManager.instance.on(RichEventDefine.UPDATE_CIRCLE, (data: { index: number, times: number }) => {
            this.updateCircle(data);
        }, this);

        RichSlotEventManager.instance.on(RichEventDefine.WIN_DIALOG_CLOSE_CALLBACK, () => {
        ...
        RichSlotEventManager.instance.on(RichEventDefine.WIN_DIALOG_OPEN_CALLBACK, () => {
            this.bigWinOpenCallback();
        }, this);
        RichSlotEventManager.instance.on(RichEventDefine.CHECK_SERVER_SLOT_DATA, this.checkServerBetData, this);
        RichSlotEventManager.instance.on(RichEventDefine.WHEEL_COLLIDE_BOTTOM, this.collideBottom, this);
        RichSlotEventManager.instance.on(RichEventDefine.SLOTS_SPEED_UP_START, this.turnStageSpeedUp, this);
    }
    ...
    private collideBottom(event) {
        RichSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.END_ROLL);
    }
    ...
    private drawPayAlllineTwo(callback: Function) {
        let tsRichView: RichView = <RichView>this.viewRootNode.getComponent("RichView");
        if (null != RichProxy.instance.gameOverData && RichProxy.instance.gameOverData.totalPay > 0 && RichProxy.instance.gameOverData.winType <= RichWinType.WIN) {
            this.tsSlotsBottomBtn.setWinNum({ num: RichProxy.instance.gameOverData.totalPay, isChange: true });
            this.tsSlotsBottomBtn.setWinTitle(false);
        }
        tsRichView.updateSelfMoney();
        RichLogManager.instance.info("开始绘制所有赔付线");
     ...
     */
    public checkIsClick() {
        let tsCommonTop: RichCommonTop = <RichCommonTop>this.tsRichView.topNode.getComponent(RichCommonTop);
        let isJump = tsCommonTop.checkShowLevelRoad(false);
        if (RichProxy.instance.gameOverData.free == 0
            && !RichProxy.instance.gameOverData.freeGameSettle
            && !isJump) {
            this.tsRichView.isCanClick();
        }
    }
    ...
    */
    public checkIsClickFront() {
        let tsCommonTop: RichCommonTop = <RichCommonTop>this.tsRichView.topNode.getComponent(RichCommonTop);
        let isJump = tsCommonTop.checkShowLevelRoad(false);
        if (null != RichProxy.instance.gameOverData
            && RichProxy.instance.gameOverData.free == 0
            && !RichProxy.instance.gameOverData.freeGameSettle
            && !isJump
            && RichProxy.instance.gameOverData.winType <= RichWinType.WIN
        ) {
            this.tsRichView.isCanClick();
        }
    }
    ...
    private playEveryOneSound(id: number) {
        let path = RichSoundsType.EVERY_LINE;
        // switch (id) {
        //     case RichIconType.FREE:
        //         path = RichSoundsType.FREE;
        //         break;
        // }
        RichSlotAudioManager.instance.playSound(BundleType.STONE_AGE, path);
    }
    ...
    public drawPayEveryOneLine(callbackYes: Function) {
        if (this.tsSlotsBottomBtn.getIsAutoBet()
            || RichProxy.instance.gameOverData.gameType == RichGameType.FREE
            || RichProxy.instance.freeSpinCount != 0) {
            if (callbackYes) {
                callbackYes();
            }
        }
            ...
            this.tsRichView.isCanClick();
            RichLogManager.instance.info("开始分别绘制每条赔付线");
            ...
            if (0 == linesDataArr.length) {
                RichLogManager.instance.info("没有赔付线");
                if (callbackYes) {
                    callbackYes();
                }
            }
    ...
    private drawLine(linesDataArr: Array<{ linePoints: Array<cc.Vec2>, colorHex: number }>, opacity: number) {
        ...
        for (let index = 0; index < linesDataArr.length; index++) {
            ...
            let colorRGB: { r: number, g: number, b: number } = RichUtil.hexToRGB(lineOne.colorHex);
    ...
    private drawRect(rectArr: Array<{ colorHex: number, node: cc.Node }>, opacity: number) {
        ...
        for (let index = 0; index < rectArr.length; index++) {
            ...
            let tsLuckyCloverItem: RichItem = <RichItem>rectOne.node.getComponent("RichItem");
            ...
            let colorRGB: { r: number, g: number, b: number } = RichUtil.hexToRGB(rectOne.colorHex);
            graphics.strokeColor = cc.color(colorRGB.r, colorRGB.g, colorRGB.b, opacity);
            graphics.rect(
                vec2.x - 76,
                vec2.y - 65.5,
    ...
    public cleanResultAnim() {
        RichLogManager.instance.info("清空所有动画");
        cc.Tween.stopAllByTarget(this.tweenObj);
        cc.Tween.stopAllByTarget(this.particleNode);
        cc.Tween.stopAllByTarget(this.particleOneNode);
        cc.Tween.stopAllByTarget(this.particleTwoNode);
        ...
        //讲隐藏的图片透明度还原
        this.tsRichView.setOpacityFull();
        this.tsSlotsBottomBtn.setNotOperationAnim();
        let tsRichView: RichView = <RichView>this.viewRootNode.getComponent("RichView");
        tsRichView.tsRichOperate.updateBet(false, false);
    }
    ...
    public checkServerData(callbackYes: Function, callbackNo: Function) {
        ...
        if (RichProxy.instance.gameOverData == null) {
            RichLogManager.instance.log("未收到服务器数据");
            if (RichProxy.instance.freeSpinCount == 0) {
                let tsRichView: RichView = <RichView>this.viewRootNode.getComponent("RichView");
                this.tsSlotsBottomBtn.setIsAutoBet(false);
                if (!this.tsSlotsBottomBtn.getIsAutoBet()) {
                    tsRichView.tsRichOperate.setButtonCanClick();
                }
            ...
            let tsRichView: RichView = <RichView>this.viewRootNode.getComponent("RichView");
            tsRichView.restoreSelfMoney();
            RichProxy.instance.initAddCircleStartIndex();
            if (callbackNo) {
                callbackNo();
            }
        }
    ...
    private playOneWinAnim(node: cc.Node) {
        let tsLuckyCloverItem: RichItem = <RichItem>node.getComponent("RichItem");
    ...
    public isAutoSpin() {
        if (this.tsSlotsBottomBtn.getIsAutoBet()) {
            ...
            this.tsSlotsBottomBtn.setSpinStatus(SpinBtnStatus.SPIN);
            if (RichProxy.instance.gameOverData != null && !RichProxy.instance.gameOverData.freeGameSettle) {
                RichLogManager.instance.info("开始分别画线");
                // this.drawPayEveryOneLine();
            }
            else {
                RichLogManager.instance.info("进入清理阶段");
    ...
    private handlePayLinesData(): Array<{
        ...
        // let linesArr = RichProxy.instance.getPayLinesData();
        ...
        //         let combo = RichProxy.instance.getComboByLinesId(line.id);
        //         let num = combo;
        ...
        //         }
        //         linePosArr.push({ linePoints: _linePosArr, colorHex: line.color, rectArr: _rectArr, targetId: RichProxy.instance.getTargetIdByLinesId(line.id) });
        //     }
        // }
        // return linePosArr;
        return null;
    ...
    public scrollStopOne(index: number) {
        ...
        let tsRichScroll: RichScroll = <RichScroll>this.listScroll[index].getComponent("RichScroll");
        //停止当前加圈动画
        tsRichScroll.stopAddCircleSpine();
        if (index >= RichProxy.instance.getAddCircleStartIndex() - 1 &&
            -1 != RichProxy.instance.getAddCircleStartIndex() &&
            index != this.scrollNodeArr.length - 1 &&
            RichProxy.instance.gameOverData != null) {
            RichSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.EXTEND_ROLL);
            let tsNextLuckyCloverScroll: RichScroll = <RichScroll>this.listScroll[index + 1].getComponent("RichScroll");
            if (null != tsNextLuckyCloverScroll) {
                //播放下一个轴的加圈动画
                tsNextLuckyCloverScroll.playAddCircleSpine();
            }
        }
        let luckyCloverTimestamp = tsRichScroll.playFreeAnim();
        ...
        if (this.currScrollStopCount == this.scrollNodeArr.length) {
            RichSlotEventManager.instance.emit(RichEventDefine.SLOTS_TURN_ALL_END);
            ...
            if (currTimestamp >= this.scrollAnimTimestamp) {
                ...
                //等待免费转动画自然播完再结算
                this.delayResultTimerId = this.setTimeOut(() => { RichAcitonManager.instance.addAction({ type: RichActionType.RESULT }); }, (this.scrollAnimTimestamp - currTimestamp) / 1000);
            }
    ...
    */
    ...
    public setWinNum() {
        let time: number;
        switch (RichProxy.instance.gameOverData.winType) {
            case RichWinType.BIG_WIN:
                time = 4.7;
                break;
            case RichWinType.MEGA_WIN:
                time = 4.7;
                break;
            case RichWinType.SUPER_WIN:
                time = 4.7;
                break;
            case RichWinType.HUGE_WIN:
                time = 4.7;
                break;
        }
        this.tsSlotsBottomBtn.setWinNum({ num: RichProxy.instance.gameOverData.totalPay, isChange: true, isBigWin: true, bigWinTime: time });
        this.tsSlotsBottomBtn.setWinTitle(false);
        let tsRichView: RichView = <RichView>this.viewRootNode.getComponent("RichView");
        tsRichView.updateSelfMoney();
    }
    ...
    */
    ...
    public playAllItemAnim(tableId: number) {
        for (let index = 0; index < this.scrollNodeArr.length; index++) {
            ...
            for (let i = 0; i < scrollNode.children.length; i++) {
                ...
                let tsLuckyCloverItem: RichItem = <RichItem>item.getComponent("RichItem");
    ...
    public checkLevelRoad(callback: Function): boolean {
        let tsRichView: RichView = <RichView>this.viewRootNode.getComponent("RichView");
        let tsCommonTop: RichCommonTop = <RichCommonTop>tsRichView.topNode.getComponent(RichCommonTop);
