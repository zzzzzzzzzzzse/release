...
import ProgressBarEx, { ProgressBarEvent } from "../BaseFrame/UI/Component/RichProgressBarEx";
...
@ccclass
export default class RichCommonTop extends RichComponentBase {
    @property(ButtonEx)
    ...
    private goldLabel: RichLabelEx = null;//金币
    @property(RichLabelEx)
    private levelLabel: RichLabelEx = null;//等级
    ...
    private addHandler(): void {
        ...
        RichSlotEventManager.instance.on(RichEventDefine.CLOSE_MENU_DIALOG, this.onMenuClose, this);
        RichSlotEventManager.instance.on(RichEventDefine.USERINFO_CHANGED, this.updateAvatar, this);
        RichSlotEventManager.instance.on(RichEventDefine.UPDATE_PLAYER_GOLD, this.playGoldChanged, this);
        RichSlotEventManager.instance.on(RichEventDefine.COMMON_TOP_STATUS_CHANGE, (status: CommonTopStatus) => {
    ...
    public updatePlayerGold(isDigit: boolean = true, descGold: number = -1): void {
        ...
        this.goldLabel.label.string = RichUtil.formateCoinStr(gold);
    }
    ...
    private onAvatar(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            RichTipsManager.instance.showTips(this.tipKey);
        }
        else {
            RichDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/InformationDialog');
        }
    }
    ...
    private onShop(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            RichViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/ShopView', null, this.isHall, ViewCloseAnimType.ONE, ViewOpenAnimType.ONE);
        }
    }
    ...
    private onVip(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            RichDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/PrimeDialog');
        }
    ...
    private onSaveMoney(): void {
        ...
        const unlock = RichUtil.checkFunctionOpen(FunctionKey.BANK_PIG);
        if (!unlock) {
            return;
        }
        RichViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/BankView', null, this.isHall);
    }
    ...
    private onMenu(): void {
        this.showMenu = true;
        RichDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/MenuDialog', this.tipKey);
