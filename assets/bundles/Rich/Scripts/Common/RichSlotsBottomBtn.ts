...
import { BundleType, CommonTopStatus, GuideGroup, MaxStatus, SlotsViewScaleStatus, SoundsType, SpinAnimStatus, SpinBtnStatus } from "../BaseFrame/Const/RichCommonDefine";
...
import RichTipsManager from "../BaseFrame/Manager/RichTipsManager";
...
import RichCommonTop from "./RichCommonTop";
// import GuideProxy from "./GuideProxy";
import PlayerProxy from "./RichPlayerProxy";

const { ccclass, property } = cc._decorator;

@ccclass
export default class RichSlotsBottomBtn extends RichComponentBase {
    @property(ButtonEx)
    ...
    private updateBetGold(): void {
        this.betLabelNode.getComponent(cc.Label).string = '' + RichProxy.instance.betGold;
    }

    onDisable() {
        RichLogManager.instance.log('清理SlotsBottomBtn的所有延迟调用');
        this.clearAllEvent();
    ...
    */
    ...
    public cancleNotOperationAnim() {
        RichLogManager.instance.log("[SlotsBottomBtn] cancleNotOperationAnim");
    ...
    private onClickAddBet() {
        ...
        if (RichProxy.instance.betGold >= 10000) {
            RichProxy.instance.betGold = 1000;
            this.updateBetGold();
            return;
        }
        RichProxy.instance.betGold += 1000;
    ...
    private onClickSubBet() {
        if (RichProxy.instance.betGold <= 1000) {
            RichProxy.instance.betGold = 10000;
            this.updateBetGold();
            return;
        }
        RichProxy.instance.betGold -= 1000;
    ...
    private onLongPressSubBet() {
        ...
        RichSlotEventManager.instance.emit(RichEventDefine.BET_GOLD_CHANGE, { index: this.currBetRangeIndex });
    ...
    public onClickSpin(testData?: string) {
        RichLogManager.instance.log("[SlotsBottomBtn] onClickSpin")
        if (!PlayerProxy.instance.roomId) {
            RichLogManager.instance.err("房间已销毁，不能投注")
            return;
        }
        if (PlayerProxy.instance.gold < RichProxy.instance.betGold) {
            return;
        }
        ...
        //先把钱扣了
        let tsCommonTop: RichCommonTop = <RichCommonTop>this.topNode.getComponent(RichCommonTop);
        PlayerProxy.instance.gold -= RichProxy.instance.betGold;
        tsCommonTop.updatePlayerGold(true, PlayerProxy.instance.gold);
        RichProxy.instance.randGameOverData();
    ...
    public freeSpin() {
        ...
        //播放音效
        RichSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.SPIN);
        if (this.getSpinStatus() == SpinBtnStatus.SPIN) {
            this.stopSpinSpineAnim();
        }
        this.cancleNotOperationAnim();
   ...
   */
    ...
    */
    private onLongPressSpinEndCallback() {
        RichLogManager.instance.log("[SlotsBottomBtn] onLongPressSpinEndCallback");
        // if (PlayerProxy.instance.gameId == parseInt(MessageType.CLASSIC777_MSG)) {
        //     const guideInfo = GuideProxy.instance.getCurrGuideInfo();
        //     if (guideInfo && guideInfo.group == GuideGroup.GUIDE_PLAY_GAME) {
        //         return;
        ...
        this.stopSpinSpineAnim();
        this.setNotOperationAnim();
        RichDeviceManager.instance.stopShake();
        //回调
    ...
    public playSpinSpineAnim(animStatus?: SpinAnimStatus, repeat: number = 0, callback?: Function) {
        ...
        if (animStatus == SpinAnimStatus.NULL) {
            RichLogManager.instance.log("stopBtnNull");
            this.stopSpinSpineAnim();
            return;
        }
    ...
    public setSpinStatus(status: SpinBtnStatus) {
        ...
        RichLogManager.instance.log("设置按钮状态为：", status);
    ...
    public async setMaxBetStatus(status: MaxStatus) {
        ...
        switch (status) {
            case MaxStatus.MAX:
                ...
                betRes = await RichSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt_max", cc.SpriteFrame);
            ...
            case MaxStatus.NOT_MAX:
                ...
                betRes = await RichSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt", cc.SpriteFrame);
                if (null != this.betTitleNode && this.betTitleNode.isValid) {
                    this.betTitleNode.getComponent(cc.Sprite).spriteFrame = betRes;
                }
                break;
    ...
    public setButtonCanNotClick() {
        RichLogManager.instance.log("[SlotsBottomBtn] setButtonCanNotClick");
        this.isCanClick = false;
        this.spinBigButtonEx.setDisable();
        this.spinSmallButtonEx.setDisable();
        RichSlotEventManager.instance.emit(RichEventDefine.COMMON_TOP_STATUS_CHANGE, CommonTopStatus.BAN);
    }
    ...
    public setButtonCanClick() {
        RichLogManager.instance.log("[SlotsBottomBtn] setButtonCanClick");
    ...
    public setWinNum(data: { num: number, isChange: boolean, isBigWin?: boolean, bigWinTime?: number, initNum?: number }) {
        ...
        if (data.isChange) {
            ...
            this.winNumLabelNode.getComponent(RichLabelEx).digitEffect({ score: data.num, time: time, bit: 0, isScale: true, initNum: data.initNum });
        }
        else {
            this.winNumLabelNode.getComponent(RichLabelEx).stopDigitEffect();
    ...
    public btnCanNotTips() {
        ...
        RichTipsManager.instance.showTips('tips_cant_click');
        return false;
    }
}
