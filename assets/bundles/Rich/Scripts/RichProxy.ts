...
import RichConfigProxy from "./Common/RichConfigProxy";
import { RichBetResponse, RichGameOverNotify } from "./PBClass/RichGame";
import { RichGameType, RichIconType, RichSceneStatus } from "./RichDefine";
import { RichEventDefine2 } from "./RichEventDefine2";
const { ccclass, property } = cc._decorator;

@ccclass
export default class RichProxy {
    private linesDataArr: Array<RichLines> = [];//赔付线数据
    private _times: string;//局号
    private _gameOverData: RichGameOverNotify;//游戏结束数据
    private _betResponse: RichBetResponse;//下注返回数据
    private handlePayTableArr: Array<Array<number>> = [[], [], [], [], []];//表盘数据
    private _freeSpinCount: number = 0;//剩余免费转次数
    private _sceneStatus: RichSceneStatus = RichSceneStatus.NORMAL;//场景状态
    private _freeBet: number = 0;//免费转投注金额
    private addCircleStartIndex: number = -1;//需要加圈的起始下标（-1为不加圈）
    private _nextGameType: RichGameType = null;//下一局游戏类型
    private static _instance: RichProxy

    private _betGold: number = 1000;

    public static get instance() {
        if (RichProxy._instance == null) {
            RichProxy._instance = new RichProxy();
        }
        return RichProxy._instance;
    }

    private constructor() {
        this._gameOverData = new RichGameOverNotify();
        this._betResponse = new RichBetResponse();
    ...
    public set gameOverData(gameOverData) {
        ...
        RichSlotEventManager.instance.emit(RichEventDefine2.START_SPIN);
    }

    public randGameOverData(): void {
        this._gameOverData.times = '101';
        this._gameOverData.paytable = JSON.stringify({});
        let randNum = RichUtil.getRandomInt(0, 10);
        if (randNum < 5) {
            this._gameOverData.totalPay = 0;
        }
        else {
            randNum = RichUtil.getRandomInt(5, 20);
    ...
    public set freeSpinCount(count) {
        ...
        RichSlotEventManager.instance.emit(RichEventDefine2.UPDATE_FREE_COUNT);
    }
    ...
    public set sceneStatus(sceneStatus: RichSceneStatus) {
    ...
    public set nextGameType(type: RichGameType) {
    ...
    */
    public clean() {
        ...
        this.sceneStatus = RichSceneStatus.NORMAL;
    }
    ...
    private handleTable() {
        ...
        RichLogManager.instance.info("this.handlePayTableArr:", this.handlePayTableArr);
    }
        ...
        for (let index = 0; index < this.handlePayTableArr.length; index++) {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (payTableObj[num].scatter_circle != 0 && payTableObj[num].freegame[map[num]] > 0) {
                    ...
                    RichSlotEventManager.instance.emit(RichEventDefine2.UPDATE_CIRCLE, { index: index + 1, times: payTableObj[num].scatter_circle })
                    break;
                }
            }
            if (isAdd) {
    ...
    private setPayLines() {
        ...
        for (let index = 0; index < this._gameOverData.lineWin.length; index++) {
            ...
            this.linesDataArr.push(RichConfigProxy.instance.getConfigData('Level15_Lines')[lines.lineid] as RichLevel15Lines);
        }
    }
     ...
     * @returns 
     */
    public getPayLinesData(): Array<RichLines> {
