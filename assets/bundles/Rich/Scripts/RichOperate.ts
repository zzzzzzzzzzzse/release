...
@ccclass
export default class RichOperate extends RichComponentBase {
    @property(ButtonEx)
    ...
    private tsRichTurntable: RichTurnTable = null;
    ...
    private addEvent() {
        ...
        RichSlotEventManager.instance.on(RichEventDefine.STOP_AUTO_SPIN, this.onClickStopAutoSpin, this);
        RichSlotEventManager.instance.on(RichEventDefine.BET_ERROR, () => {
            this.setButtonCanClick();
            this.onClickStopAutoSpin();
        }, this);
        RichSlotEventManager.instance.on(RichEventDefine.UPDATE_FREE_COUNT, () => {
            this.freeCountLabelNode.getComponent(cc.Label).string = RichProxy.instance.freeSpinCount.toString();
        }, this);
        ...
        this.testBtn.clickCallback = () => {
            ...
            // DialogManager.instance.openDialog(BundleType.STONE_AGE, "Prefabs/FreeDialog/RichFreeResultDialog", {
            //     num: 500,
            //     callback: () => {
            //         // this.close();
            //     }
            // });
            // ViewManager.instance.openView(BundleType.STONE_AGE, 'Prefabs/MiniGameView/RichMiniView', { data: {}, isKill: false }, false, ViewCloseAnimType.ONE, ViewOpenAnimType.ONE);
            // DialogManager.instance.openDialog(BundleType.STONE_AGE, "Prefabs/FreeDialog/RichFreeDialog", {
            //     num: 5,
            //     callback: () => {
            //     }
            // });
    ...
    public onClickSpin(testData?: string) {
        //设置lastwin
        if (RichProxy.instance.gameOverData && RichProxy.instance.gameOverData.totalPay && 0 != RichProxy.instance.gameOverData.totalPay) {
        ...
        this.tsRichTurntable.clearDelayResultTimerId();
        let action = RichAcitonManager.instance.getNextAction();
        if (null != action && action.type == RichActionType.RESULT) {
    ...
    */
    public onClickStopAutoSpin() {
        ...
        if (this.tsRichView.tsRichTurntable.getScrollStatus() == TurntableStatus.STOP) {
            this.setButtonCanClick()
        };
    }
    ...
    */
    private onClickReturn() {
        ...
        if (tsCommonTop.getStatus() == CommonTopStatus.BAN) {
            ...
            this.node.destroy();
            // const bundle = cc.assetManager.getBundle('Rich')
            // bundle.releaseAll();
        }
    }

    private onClickRule() {
        RichDialogManager.instance.openDialog(BundleType.STONE_AGE, "Prefabs/RuleDialog/RichRuleDialog");
    }
   ...
   */
    ...
    public updateBet(isChange: boolean = false, isPlaySound: boolean = true) {
        let bet = null;
        if (RichProxy.instance.freeSpinCount != 0) {
            bet = RichProxy.instance.freeBet;
        }
    ...
    public setCurrGold(num: number) {
        this.currGold = num;
        RichLogManager.instance.info("设置gold：", this.currGold);
    }
