...
import PageViewEx from "../BaseFrame/UI/Component/RichPageViewEx";
import RichDialogBase from "../BaseFrame/UI/Dialog/RichDialogBase";

const { ccclass, property } = cc._decorator;

@ccclass
export default class RichRuleDialog extends RichDialogBase {
    @property(PageViewEx)
    ...
    public renderItem(item: cc.Node, idx: number) {
        // let tsRichRuleItem: RichRuleItem = <RichRuleItem>item.getComponent("RichRuleItem");
        // LogManager.instance.info("this.payTableArr:", this.payTableArr);
        // tsRichRuleItem.updateItem(this.payTableArr[idx]);
    ...
    private updateFooter(percent: number, index: number) {
        RichLogManager.instance.log("[RichRuleDialog] updateFooter:", percent, ",", index);
