...
import PoseidonDialogManager from "../Manager/PoseidonDialogManager";
...
export default class PoseidonAppleSdk {
    private static _instance: PoseidonAppleSdk = null;
     ...
     * 获取单例
     */
    public static get instance(): PoseidonAppleSdk {
        if (!this._instance) {
            this._instance = new PoseidonAppleSdk();
        }
    ...
    private notSupportLogin(): void {
        PoseidonDialogManager.instance.openCommonDialog('cant_bind_apple');
    }
    ...
    private loginCallback(resultStr: string): void {
        ...
        PoseidonSlotEventManager.instance.emit(PoseidonEventDefine.APPLE_LOGIN_SUCCESS);
    }
    ...
    private buyItemsSuccess(resultStr: string): void {
        ...
        data['payId'] = payId;
        PoseidonLogManager.instance.log(payId + '购买成功');
        if (this.isRestore) {
            this.isRestore = false;
