import PoseidonHttpManager from "./PoseidonHttpManager";
import PoseidonLogManager from "./PoseidonLogManager";

export default class PoseidonFileManager {
    private static _instance: PoseidonFileManager = null;
     ...
     * 获取单例
     */
    public static get instance(): PoseidonFileManager {
        if (!this._instance) {
            this._instance = new PoseidonFileManager();
        }
    ...
    public downloadBinaryFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        if (count > this.maxCount) {
            ...
            PoseidonLogManager.instance.log('达到最大下载数量，进入排队队列');
            return;
        }
    ...
    public downloadTxtFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        this.requestList[url] = callback;
        const download = url => {
            PoseidonHttpManager.instance.sendRequestGet(url, response => {
                const callback = this.requestList[url];
    ...
    private downloadWaitList(): void {
        ...
        PoseidonLogManager.instance.log('从队列中下载文件:', info.url);
        if (info.isBinary) {
