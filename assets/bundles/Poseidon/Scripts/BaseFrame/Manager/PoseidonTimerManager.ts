 ...
 * 计时器
 */
export default class PoseidonTimerManager {
    private static _instance: PoseidonTimerManager
    public static get instance() {
        if (PoseidonTimerManager._instance == null) {
            PoseidonTimerManager._instance = new PoseidonTimerManager();
        }
        return PoseidonTimerManager._instance;
    }
    ...
    public setInterval(callback: Function, interval: number = 0, repeat: number = null): number {
        let count = 0;
        if (repeat < 0) {
            PoseidonLogManager.instance.err("Repeat must be equal or greater than 0");
            return;
        }
        if (null == repeat) {
            repeat = -1;
