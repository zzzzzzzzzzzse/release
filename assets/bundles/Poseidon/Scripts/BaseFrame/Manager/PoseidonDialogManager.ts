...
import PoseidonDialogBase from "../UI/Dialog/PoseidonDialogBase";
...
@ccclass
export default class PoseidonDialogManager {
    ...
    private static _instance: PoseidonDialogManager;
    public static get instance() {
        if (PoseidonDialogManager._instance == null) {
            PoseidonDialogManager._instance = new PoseidonDialogManager();
        }
        return PoseidonDialogManager._instance;
    }
    ...
    public openDialog(abundleName: string, path: string, data?: {}): void {
        if (this.isPopExist(path)) {
            PoseidonLogManager.instance.log("重复打开弹框:", abundleName, "->", path);
        }
        else {
        ...
        if (null != this.dialogMap[path]) {
            PoseidonLogManager.instance.log("弹框数据还在:", abundleName, "->", path);
            if (this.dialogMap[path].node != null) {
                PoseidonLogManager.instance.log("弹框节点还在:", abundleName, "->", path);
                if (null == this.dialogCon) {
                    this.dialogCon = cc.find("Canvas/dialogNode");
                }
                let tsDialogBase: PoseidonDialogBase = <PoseidonDialogBase>this.dialogMap[path].node.getComponent("DialogBase");
            ...
            PoseidonLogManager.instance.log("没有弹框数据:", abundleName, "->", path);
            this.dialogMap[path] = {};
    ...
    public closeDialog(path?: string, isDestroy: boolean = true) {
        ...
        let tsDialogBase: PoseidonDialogBase = <PoseidonDialogBase>dialogNode.getComponent("DialogBase");
    ...
    private async loadModule(abundleName: string, path: string, data?: {}) {
        PoseidonLogManager.instance.log("开始加载弹框资源:", abundleName, "->", path);
        //这里放加载动画
        let res: cc.Prefab = await PoseidonSlotResManager.instance.loadByBundle(
            abundleName,
            path,
            cc.Prefab,
            (finish, total) => {
        ...
        PoseidonLogManager.instance.log("弹框资源加载成功:", abundleName, "->", path);
        //这里关闭加载动画
        var newNode = cc.instantiate(res);
        if (null == this.dialogCon) {
            this.dialogCon = cc.find("Canvas/dialogNode");
        }
        let tsDialogBase: PoseidonDialogBase = <PoseidonDialogBase>newNode.getComponent("DialogBase");
        tsDialogBase.setAbName(abundleName);
        tsDialogBase.setPath(path);
        tsDialogBase.setData(data);
    ...
    private isPopExist(path: string) {
        PoseidonLogManager.instance.log("已有弹框：", this.openList);
        for (let index = 0; index < this.openList.length; index++) {
            const element = this.openList[index];
            if (element == path) {
                return true;
    ...
    public openCommonDialog(popupKey: string, confirmCallback: Function = null, cancelCallback: Function = null, replaceParams: any[] = []): void {
        ...
        PoseidonDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonDialog', data);
    }
    ...
    public openCommonPrompts(titleLanguageKey: string, detailLanguageKey: string, wPos: cc.Vec2, params: any[] = [], refWPos: cc.Vec2 = null): void {
        ...
        PoseidonDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonPrompts', data);
    }
    ...
    public closeCommonPrompts(): void {
        PoseidonDialogManager.instance.closeDialog('Prefabs/Common/CommonPrompts');
    }
    ...
    public checkCommonDialogIsExist(): boolean {
        const dialogIsExist = PoseidonDialogManager.instance.isPopExist('Prefabs/Common/CommonDialog');
    ...
    public closeAll() {
        while (0 != this.openList.length) {
            ...
            if (null == dialogNode) {
                ...
                PoseidonLogManager.instance.info("closeDialog:", path);
            }
        }
    }
    ...
    public showCommonReward(itemList: any[]): void {
        ...
        PoseidonDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonReward', itemList);
    }
    ...
    public showCommonItemList(itemList: any[], wPos: cc.Vec2): void {
        ...
        PoseidonDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonItemList', { itemList: itemList, wPos: wPos });
    }
}
