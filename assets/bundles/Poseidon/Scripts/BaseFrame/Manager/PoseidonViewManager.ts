...
import { BundleType, ViewCloseAnimType, ViewLevel, ViewOpenAnimType } from "../Const/PoseidonCommonDefine";
import PoseidonViewBase from "../UI/View/PoseidonViewBase";
...
@ccclass
export default class PoseidonViewManager {
    ...
    private static _instance: PoseidonViewManager
    public static get instance() {
        if (PoseidonViewManager._instance == null) {
            PoseidonViewManager._instance = new PoseidonViewManager();
        }
        return PoseidonViewManager._instance;
    }
    ...
    public openView(
        ...
        PoseidonTimerManager.instance.clearInterval(this.delayReleaseIDMap[abundleName]);
    ...
    public back(
        ...
        for (let index = this.openListArr.length - 1; index >= 0; index--) {
            ...
            if (element.level == ViewLevel.LEVEL_ONE) {
                obj = element;
                PoseidonLogManager.instance.log("openListArrBackOver：", this.openListArr);
                break;
            }
            else {
                ...
                PoseidonLogManager.instance.log("openListArr：", this.openListArr);
            }
        }
        if (null == obj) {
            PoseidonLogManager.instance.err("back view is null");
            return;
        }
        if (!isOpenLast) {
            return;
        }
        PoseidonTimerManager.instance.clearInterval(this.delayReleaseIDMap[obj.abundleName]);
        this.opening(
            obj.abundleName,
            obj.path,
            obj.data,
    ...
    private handleView(
        ...
        let tsViewBase: PoseidonViewBase = <PoseidonViewBase>view.getComponent("ViewBase");
    ...
    private destroyLastView(isDestroy: boolean, closeAnimType: ViewCloseAnimType) {
        if (null != this.lastViewPath) {
            ...
            if (null != lastNode) {
                let tsOtherViewBase: PoseidonViewBase = <PoseidonViewBase>lastNode.getComponent("ViewBase");
                ...
                tsOtherViewBase.setClose(isDestroy, () => {
                    if (isGame && isDestroy) {
                        this.delayReleaseIDMap[_abundleName] = PoseidonTimerManager.instance.setInterval(() => {
                            PoseidonSlotResManager.instance.releaseBundle(_abundleName);
                            PoseidonLogManager.instance.info("释放成功:", this.delayReleaseIDMap[_abundleName], " name:", _abundleName);
                        }, 0, 0);
                        PoseidonLogManager.instance.info("开始释放：", _abundleName, "计时器：", this.delayReleaseIDMap[_abundleName]);
                    }
                }, closeAnimType);
    ...
    public returnHall() {
        ...
        this.clean();
        PoseidonSlotResManager.instance.cleanGameBundle();
    ...
    public returnLogin() {
        ...
        this.clean();
        PoseidonSlotResManager.instance.cleanAllBundle();
    ...
    private clean() {
        for (const key in this.viewMap) {
            ...
            if (null != element.node) {
                let tsOtherViewBase: PoseidonViewBase = <PoseidonViewBase>element.node.getComponent("ViewBase");
