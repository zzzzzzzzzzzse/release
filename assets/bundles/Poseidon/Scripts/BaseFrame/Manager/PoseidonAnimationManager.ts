 ...
 * 动画管理
 */
export default class PoseidonAnimationManager {
    private backPlayMap: { [key: number]: { animation: cc.Animation } } = {};//需要在切换到后台的时候依旧播放的动画数组
    private static _instance: PoseidonAnimationManager;
    private id: number = 0;//每个动画的唯一id
    public static get instance() {
        if (PoseidonAnimationManager._instance == null) {
            PoseidonAnimationManager._instance = new PoseidonAnimationManager();
        }
        return PoseidonAnimationManager._instance;
    }

    public constructor() {
        PoseidonSlotEventManager.instance.on(PoseidonEventDefine.QUIE_GAME_SHOW, (interval: number) => {
    ...
    public async playAnimation(data: {
        ...
        let res: cc.Prefab = await PoseidonSlotResManager.instance.loadByBundle(data.animBundle, data.animPath);
