...
export default class PoseidonSlotResManager {
    private static _instance: PoseidonSlotResManager
    public static get instance() {
        if (PoseidonSlotResManager._instance == null) {
            PoseidonSlotResManager._instance = new PoseidonSlotResManager();
        }
        return PoseidonSlotResManager._instance;
    }
    ...
    public async loadBundle(aBName: string): Promise<cc.AssetManager.Bundle> {
        let startTime: number = PoseidonUtil.getTimeStamp();
        let endTime: number;
        return new Promise<any>((resolve, reject) => {
            cc.assetManager.loadBundle(aBName, (err, bundle) => {
                if (err) {
                    PoseidonLogManager.instance.log("loadBundleFail:", aBName);
                    reject(err.message || err);
                }
                else {
                    endTime = PoseidonUtil.getTimeStamp();
                    PoseidonLogManager.instance.log("loadBundleSucc:", aBName, " costTime:", endTime - startTime, "ms");
                    resolve(bundle);
                }
            });
        })
    ...
    public async loadByBundle(aBName: string, path: string, type?: typeof cc.Asset, onProgress?: (finish: number, total: number) => void): Promise<any> {
        ...
        type ||= cc.Asset;
        if (null == abundle) {
            PoseidonLogManager.instance.err("abundle is null:", aBName);
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.load(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = PoseidonUtil.getTimeStamp();
                        PoseidonLogManager.instance.log("loadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        PoseidonLogManager.instance.err(err)
                        reject(err.message || err);
                    }
                }
            );
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.loadDir(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = PoseidonUtil.getTimeStamp();
                        PoseidonLogManager.instance.log("loadDirByBundleSucc:", aBName, "->", dir, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        reject(err.message || err);
    ...
    public releaseBundle(aBName: string) {
        ...
        if (null != bundle) {
            ...
            PoseidonLogManager.instance.log('释放bundle：', aBName);
            ...
            abundle.preloadDir(
                ...
                (err, asset) => {
                    if (null != err) {
                        ...
                        endTime = PoseidonUtil.getTimeStamp();
                        PoseidonLogManager.instance.log("preloadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                }
            );
