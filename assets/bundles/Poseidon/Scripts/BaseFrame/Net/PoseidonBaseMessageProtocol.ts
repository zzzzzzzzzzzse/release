...
import PoseidonProtocolBase from "./PoseidonProtocolBase";

export default class PoseidonBaseMessageProtocol extends PoseidonProtocolBase {
    protected pbPackageName: string = "com.islet.game.proto";//当前协议包名
    protected pbName: string = "PB/BaseMessage";//pb文件名，不带扩展名

    private static _instance: PoseidonBaseMessageProtocol = null;
     ...
     * 获取单例
     */
    public static get instance(): PoseidonBaseMessageProtocol {
        if (!this._instance) {
            this._instance = new PoseidonBaseMessageProtocol();
        }
