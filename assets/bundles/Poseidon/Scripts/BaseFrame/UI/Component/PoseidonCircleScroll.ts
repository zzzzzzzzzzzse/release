import PoseidonCircleScrollItem from "./PoseidonCircleScrollItem";
...
@ccclass
@menu('自定义组件/PoseidonCircleScroll')
export default class PoseidonCircleScroll extends cc.Component {
    ...
    private createItem(index: number, isRight: boolean = true): void {
        ...
        if (!nodeInfo) {
            ...
            const node = cc.instantiate(this.itemPrefab);
            const circleScrollItem = node.getComponent(PoseidonCircleScrollItem);
