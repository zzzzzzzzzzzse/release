...
@ccclass
export default class PoseidonSpineEx extends sp.Skeleton {
    ...
    onDestroy() {
        if (BundleType.RESOURCES != this.abName) {
            PoseidonSlotResManager.instance.cleanAsset(this.abName, this.path, cc.SpriteAtlas);
        }
    }
    ...
    */
    public async playSpineAnim(data: {
        ...
        this.path = data.path;
        let res: sp.SkeletonData = await PoseidonSlotResManager.instance.loadByBundle(data.aBName, data.path, sp.SkeletonData);
        if (null == this.node || !this.node.isValid) {
            return;
        }
    ...
    */
    private mixSpine(_spine: sp.Skeleton, _mixTime: number) {
        ...
        PoseidonLogManager.instance.log("开始混合");
    ...
    public attachUtilSelf(targetNodeName: string, node: cc.Node) {
        ...
        if (null == boneNode) {
            PoseidonLogManager.instance.err("找不到挂点：", targetNodeName);
            return;
        }
