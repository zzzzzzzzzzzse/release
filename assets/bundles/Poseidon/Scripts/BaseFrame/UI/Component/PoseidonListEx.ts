...
import PoseidonListItem from './PoseidonListItem';
...
@ccclass
@disallowMultiple()
@menu('自定义组件/PoseidonListEx')
@requireComponent(cc.ScrollView)
//脚本生命周期回调的执行优先级。小于 0 的脚本将优先执行，大于 0 的脚本将最后执行。该优先级只对 onLoad, onEnable, start, update 和 lateUpdate 有效，对 onDisable 和 onDestroy 无效。
@executionOrder(-5000)
export default class PoseidonListEx extends cc.Component {
    //缓冲区域
    @property
    private area: number = 30;
    //模板类型
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            case SelectedType.SINGLE: {
                ...
                t._selectedId = val;
                if (item) {
                    listItem = item.getComponent(PoseidonListItem);
                    listItem.selected = true;
                }
                if (t._lastSelectedId >= 0 && t._lastSelectedId != t._selectedId) {
                    let lastItem: any = t.getItemByListId(t._lastSelectedId);
                    if (lastItem) {
                        lastItem.getComponent(PoseidonListItem).selected = false;
                    }
                }
                if (t.selectedEvent) {
            ...
            case SelectedType.MULT: {
                ...
                if (!item)
                    return;
                let listItem = item.getComponent(PoseidonListItem);
                if (t._selectedId >= 0)
                    t._lastSelectedId = t._selectedId;
                t._selectedId = val;
    ...
    _registerEvent() {
        ...
        t.content.on(cc.Node.EventType.POSITION_CHANGED, t._onScrolling, t, true);
    }
    //卸载事件
    _unregisterEvent() {
        let t: any = this;
        ...
        t.content.off(cc.Node.EventType.POSITION_CHANGED, t._onScrolling, t, true);
    }
    //初始化各种..
    _init() {
        let t: any = this;
    ...
    setTemplateItem(item: any) {
        ...
        let com = item.getComponent(PoseidonListItem);
        let remove = false;
        if (!com)
            remove = true;
        // if (com) {
    ...
    _onScrolling(ev: cc.Event = null) {
        ...
        if (!this._forceUpdate && this.frameCount > 0) {
            this.frameCount--;
            return;
        } else
            this.frameCount = this._updateRate;
    ...
    _calcViewPos() {
        ...
        switch (this._alignCalcType) {
            case 1://单行HORIZONTAL（LEFT_TO_RIGHT）、网格VERTICAL（LEFT_TO_RIGHT）
                ...
                this.viewLeft = (scrollPos.x < 0 ? -scrollPos.x : 0) - this.elasticLeft - this.area;
                this.viewRight = this.viewLeft + this.node.width + this.area;
            ...
            case 4://单列VERTICAL（BOTTOM_TO_TOP）、网格HORIZONTAL（BOTTOM_TO_TOP）
                ...
                this.viewBottom = (scrollPos.y < 0 ? -scrollPos.y : 0) - this.elasticBottom - this.area;
                this.viewTop = this.viewBottom + this.node.height + this.area;
    ...
    _onItemAdaptive(item) {
        if (this.checkInited(false)) {
            if (
                ...
                // cc.log("_onItemAdaptive",this._customSize[item._listId],val);
                if (this._customSize[item._listId] != val) {
                    ...
                    // });
                    this.updateAll();
                    // cc.log("_onItemAdaptive updateAll");
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            ...
            let listItem: PoseidonListItem = item.getComponent(PoseidonListItem);
    ...
    _updateListItem(listItem: PoseidonListItem) {
        if (!listItem)
            return;
        ...
        if (this._virtual) {
            ...
            for (let n: number = arr.length - 1; n >= 0; n--) {
                ...
                if (this._scrollItem && item._listId == this._scrollItem._listId)
                    continue;
                //修复删除后大小没有改变回来的问题
                // if (this._customSize[item._listId]){
                //     let val = this._sizeType ? this._itemSize.height : this._itemSize.width;
                //     this._customSize[item._listId] = val;
                // }
                item.height = this._itemSize.height;
                this._pool.put(item);
    ...
    public getShowNum(): number {
        const grap = this._layout.spacingX;
        const width = this.scrollView.node.width;
        const item = this._itemTmp;
        const num = 1 + Math.ceil((width - item.width) / (item.width + grap));
        return num;
    }
