...
import PoseidonListEx from "./PoseidonListEx";
import PoseidonTabItem from "./PoseidonTabItem";

const { ccclass, property, menu } = cc._decorator;

@ccclass
@menu('自定义组件/Tab/PoseidonTab')
export default class PoseidonTab extends PoseidonComponentBase {
    @property({
        type: [PoseidonTabItem],
        visible() {
            return !this.virtual;
        }
    })
    private items: PoseidonTabItem[] = [];
    @property({
        type: PoseidonListEx,
        visible() {
            return this.virtual;
    ...
    private list: PoseidonListEx = null;
    ...
    private virtualSelectChanged(): void {
        ...
        for (let i = 0; i < itemList.length; i++) {
            const tabItem = itemList[i].getComponent(PoseidonTabItem);
        ...
        if (selectNode) {
            const selectItem = selectNode.getComponent(PoseidonTabItem);
    ...
    private itemChanged(node: cc.Node, index: number): void {
        const tabItem = node.getComponent(PoseidonTabItem);
        if (!tabItem) {
            PoseidonLogManager.instance.warn(node.name + '必现继承TabItem');
            return;
        }
