...
import PoseidonPageViewIndicator from "./PoseidonPageViewIndicator";
 ...
 */
export default class PoseidonPageViewEx extends PoseidonComponentBase {
    ...
    @property({ type: PoseidonPageViewIndicator, tooltip: '分页脚标' })
    private indicator: PoseidonPageViewIndicator = null;//分页脚标
    //渲染事件（渲染器）
    @property({
    ...
    private createItem(index: number, isBehind: boolean = true): void {
        ...
        PoseidonLogManager.instance.log('创建子项:', index);
    ...
    private checkLeftNeddRecover(): void {
        ...
        if (this.content.x + leftNodeInfo.node.x < -this.perDistance) {
            ...
            PoseidonLogManager.instance.log('回收子项:', leftNodeInfo.index);
        }
    }
    ...
    private checkRightNeddRecover(): void {
        ...
        if (this.content.x + rightNodeInfo.node.x > this.perDistance) {
            ...
            PoseidonLogManager.instance.log('回收子项:', rightNodeInfo.index);
    ...
    public jumpToPage(page: number): void {
        if (this.loop) {
            PoseidonLogManager.instance.log('循环分页不支持跳转到指定的页数');
            return;
        }
        if (page < 0 || page >= this.totalPage) {
            PoseidonLogManager.instance.log('跳转的页数请在0-' + (this.totalPage - 1));
            return;
        }
