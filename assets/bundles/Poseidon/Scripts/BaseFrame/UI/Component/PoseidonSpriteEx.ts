...
@ccclass
@menu('自定义组件/PoseidonSpriteEx')
export default class PoseidonSpriteEx extends cc.Component {
    ...
    public async setImage(bundleName: string, path: string) {
        if (!this.sprite) {
            PoseidonLogManager.instance.log('请将此节点挂在sprite组件下面');
            return;
        }
        ...
        this.path = path;
        let res: cc.SpriteFrame = await PoseidonSlotResManager.instance.loadByBundle(bundleName, path, cc.SpriteFrame);
    ...
    public async loadRemoteImage(url: string, wdith: number = 0, height: number = 0) {
        ...
        this.path = url;
        this.isRemote = true;
        let spriteFrame = await PoseidonSlotResManager.instance.loadRemote(url, { ext: '.png' });
