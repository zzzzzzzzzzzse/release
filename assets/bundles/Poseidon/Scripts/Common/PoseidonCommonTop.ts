...
import ProgressBarEx, { ProgressBarEvent } from "../BaseFrame/UI/Component/PoseidonProgressBarEx";
...
@ccclass
export default class PoseidonCommonTop extends PoseidonComponentBase {
    @property(ButtonEx)
    ...
    private goldLabel: PoseidonLabelEx = null;//金币
    @property(PoseidonLabelEx)
    private levelLabel: PoseidonLabelEx = null;//等级
    ...
    private addHandler(): void {
        ...
        PoseidonSlotEventManager.instance.on(PoseidonEventDefine.CLOSE_MENU_DIALOG, this.onMenuClose, this);
        PoseidonSlotEventManager.instance.on(PoseidonEventDefine.USERINFO_CHANGED, this.updateAvatar, this);
        PoseidonSlotEventManager.instance.on(PoseidonEventDefine.UPDATE_PLAYER_GOLD, this.playGoldChanged, this);
        PoseidonSlotEventManager.instance.on(PoseidonEventDefine.COMMON_TOP_STATUS_CHANGE, (status: CommonTopStatus) => {
    ...
    public updatePlayerGold(isDigit: boolean = true, descGold: number = -1): void {
        ...
        this.goldLabel.label.string = PoseidonUtil.formateCoinStr(gold);
    }
    ...
    private onAvatar(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            PoseidonTipsManager.instance.showTips(this.tipKey);
        }
        else {
            PoseidonDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/InformationDialog');
        }
    }
    ...
    private onShop(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            PoseidonViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/ShopView', null, this.isHall, ViewCloseAnimType.ONE, ViewOpenAnimType.ONE);
        }
    }
    ...
    private onVip(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            PoseidonDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/PrimeDialog');
        }
    ...
    private onSaveMoney(): void {
        ...
        const unlock = PoseidonUtil.checkFunctionOpen(FunctionKey.BANK_PIG);
        if (!unlock) {
            return;
        }
        PoseidonViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/BankView', null, this.isHall);
    }
    ...
    private onMenu(): void {
        this.showMenu = true;
        PoseidonDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/MenuDialog', this.tipKey);
