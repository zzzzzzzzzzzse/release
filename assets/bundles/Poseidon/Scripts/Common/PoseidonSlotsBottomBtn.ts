...
import { BundleType, CommonTopStatus, GuideGroup, MaxStatus, SlotsViewScaleStatus, SoundsType, SpinAnimStatus, SpinBtnStatus } from "../BaseFrame/Const/PoseidonCommonDefine";
...
import PoseidonCommonTop from "./PoseidonCommonTop";
// import GuideProxy from "./GuideProxy";
import PlayerProxy from "./PoseidonPlayerProxy";

const { ccclass, property } = cc._decorator;

@ccclass
export default class PoseidonSlotsBottomBtn extends PoseidonComponentBase {
    @property(ButtonEx)
    ...
    private updateBetGold(): void {
        this.betLabelNode.getComponent(cc.Label).string = '' + PoseidonProxy.instance.betGold;
    }

    onDisable() {
        PoseidonLogManager.instance.log('清理SlotsBottomBtn的所有延迟调用');
        this.clearAllEvent();
    ...
    */
    ...
    public cancleNotOperationAnim() {
        PoseidonLogManager.instance.log("[SlotsBottomBtn] cancleNotOperationAnim");
    ...
    private onClickAddBet() {
        ...
        if (PoseidonProxy.instance.betGold >= 10000) {
            PoseidonProxy.instance.betGold = 1000;
            this.updateBetGold();
            return;
        }
        PoseidonProxy.instance.betGold += 1000;
    ...
    private onClickSubBet() {
        if (PoseidonProxy.instance.betGold <= 1000) {
            PoseidonProxy.instance.betGold = 10000;
            this.updateBetGold();
            return;
        }
        PoseidonProxy.instance.betGold -= 1000;
    ...
    private onLongPressSubBet() {
        ...
        PoseidonSlotEventManager.instance.emit(PoseidonEventDefine.BET_GOLD_CHANGE, { index: this.currBetRangeIndex });
    ...
    public onClickSpin(testData?: string) {
        PoseidonLogManager.instance.log("[SlotsBottomBtn] onClickSpin")
        if (!PlayerProxy.instance.roomId) {
            PoseidonLogManager.instance.err("房间已销毁，不能投注")
            return;
        }
        if (PlayerProxy.instance.gold < PoseidonProxy.instance.betGold) {
            return;
        }
        ...
        //先把钱扣了
        let tsCommonTop: PoseidonCommonTop = <PoseidonCommonTop>this.topNode.getComponent(PoseidonCommonTop);
        PlayerProxy.instance.gold -= PoseidonProxy.instance.betGold;
        tsCommonTop.updatePlayerGold(true, PlayerProxy.instance.gold);
        PoseidonProxy.instance.randGameOverData();
    ...
    public freeSpin() {
        ...
        //播放音效
        PoseidonSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.SPIN);
        if (this.getSpinStatus() == SpinBtnStatus.SPIN) {
            this.stopSpinSpineAnim();
        }
        this.cancleNotOperationAnim();
   ...
   */
    ...
    */
    private onLongPressSpinEndCallback() {
        PoseidonLogManager.instance.log("[SlotsBottomBtn] onLongPressSpinEndCallback");
        // if (PlayerProxy.instance.gameId == parseInt(MessageType.CLASSIC777_MSG)) {
        //     const guideInfo = GuideProxy.instance.getCurrGuideInfo();
        //     if (guideInfo && guideInfo.group == GuideGroup.GUIDE_PLAY_GAME) {
        //         return;
        ...
        this.stopSpinSpineAnim();
        this.setNotOperationAnim();
        PoseidonDeviceManager.instance.stopShake();
        //回调
    ...
    public playSpinSpineAnim(animStatus?: SpinAnimStatus, repeat: number = 0, callback?: Function) {
        ...
        if (animStatus == SpinAnimStatus.NULL) {
            PoseidonLogManager.instance.log("stopBtnNull");
            this.stopSpinSpineAnim();
            return;
        }
    ...
    public setSpinStatus(status: SpinBtnStatus) {
        ...
        PoseidonLogManager.instance.log("设置按钮状态为：", status);
    ...
    public async setMaxBetStatus(status: MaxStatus) {
        ...
        switch (status) {
            case MaxStatus.MAX:
                ...
                betRes = await PoseidonSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt_max", cc.SpriteFrame);
            ...
            case MaxStatus.NOT_MAX:
                ...
                betRes = await PoseidonSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt", cc.SpriteFrame);
                if (null != this.betTitleNode && this.betTitleNode.isValid) {
                    this.betTitleNode.getComponent(cc.Sprite).spriteFrame = betRes;
                }
                break;
    ...
    public setButtonCanNotClick() {
        PoseidonLogManager.instance.log("[SlotsBottomBtn] setButtonCanNotClick");
    ...
    public setButtonCanClick() {
        PoseidonLogManager.instance.log("[SlotsBottomBtn] setButtonCanClick");
    ...
    public setWinNum(data: { num: number, isChange: boolean, isBigWin?: boolean, bigWinTime?: number, initNum?: number }) {
        ...
        if (data.isChange) {
            ...
            this.winNumLabelNode.getComponent(PoseidonLabelEx).digitEffect({ score: data.num, time: time, bit: 0, isScale: true, initNum: data.initNum });
        }
        else {
            this.winNumLabelNode.getComponent(PoseidonLabelEx).stopDigitEffect();
