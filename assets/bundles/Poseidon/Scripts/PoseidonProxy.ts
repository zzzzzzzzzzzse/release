...
import PoseidonConfigProxy from "./Common/PoseidonConfigProxy";
import { PoseidonBetResponse, PoseidonGameOverNotify } from "./PBClass/PoseidonGame";
import { PoseidonGameType, PoseidonIconType, PoseidonSceneStatus } from "./PoseidonDefine";
import { PoseidonEventDefine2 } from "./PoseidonEventDefine2";
const { ccclass, property } = cc._decorator;

@ccclass
export default class PoseidonProxy {
    private linesDataArr: Array<PoseidonLines> = [];//赔付线数据
    private _times: string;//局号
    private _gameOverData: PoseidonGameOverNotify;//游戏结束数据
    private _betResponse: PoseidonBetResponse;//下注返回数据
    private handlePayTableArr: Array<Array<number>> = [[], [], [], [], []];//表盘数据
    private _freeSpinCount: number = 0;//剩余免费转次数
    private _sceneStatus: PoseidonSceneStatus = PoseidonSceneStatus.NORMAL;//场景状态
    private _freeBet: number = 0;//免费转投注金额
    private addCircleStartIndex: number = -1;//需要加圈的起始下标（-1为不加圈）
    private _nextGameType: PoseidonGameType = null;//下一局游戏类型
    private static _instance: PoseidonProxy

    private _betGold: number = 1000;

    public static get instance() {
        if (PoseidonProxy._instance == null) {
            PoseidonProxy._instance = new PoseidonProxy();
        }
        return PoseidonProxy._instance;
    }

    private constructor() {
        this._gameOverData = new PoseidonGameOverNotify();
        this._betResponse = new PoseidonBetResponse();
    ...
    public set gameOverData(gameOverData) {
        ...
        PoseidonSlotEventManager.instance.emit(PoseidonEventDefine2.START_SPIN);
    }

    public randGameOverData(): void {
        this._gameOverData.times = '101';
        this._gameOverData.paytable = JSON.stringify({});
        let randNum = PoseidonUtil.getRandomInt(0, 10);
        if (randNum < 5) {
            this._gameOverData.totalPay = 0;
        }
        else {
            randNum = PoseidonUtil.getRandomInt(5, 20);
    ...
    public set freeSpinCount(count) {
        ...
        PoseidonSlotEventManager.instance.emit(PoseidonEventDefine2.UPDATE_FREE_COUNT);
    }
    ...
    public set sceneStatus(sceneStatus: PoseidonSceneStatus) {
    ...
    public set nextGameType(type: PoseidonGameType) {
    ...
    */
    public clean() {
        ...
        this.sceneStatus = PoseidonSceneStatus.NORMAL;
    }
    ...
    private handleTable() {
        ...
        PoseidonLogManager.instance.info("this.handlePayTableArr:", this.handlePayTableArr);
    }
        ...
        for (let index = 0; index < this.handlePayTableArr.length; index++) {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (payTableObj[num].scatter_circle != 0 && payTableObj[num].freegame[map[num]] > 0) {
                    ...
                    PoseidonSlotEventManager.instance.emit(PoseidonEventDefine2.UPDATE_CIRCLE, { index: index + 1, times: payTableObj[num].scatter_circle })
                    break;
                }
            }
            if (isAdd) {
    ...
    private setPayLines() {
        ...
        for (let index = 0; index < this._gameOverData.lineWin.length; index++) {
            ...
            this.linesDataArr.push(PoseidonConfigProxy.instance.getConfigData('Level15_Lines')[lines.lineid] as PoseidonLevel15Lines);
        }
    }
     ...
     * @returns 
     */
    public getPayLinesData(): Array<PoseidonLines> {
