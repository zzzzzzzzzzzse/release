...
import NewBankConfigProxy from "./Common/NewBankConfigProxy";
import { NewBankGameType, NewBankIconType, NewBankSceneStatus } from "./NewBankDefine";
import { NewBankEventDefine2 } from "./NewBankEventDefine2";
import { NewBankBetResponse, NewBankGameOverNotify } from "./PBClass/NewBankGame";
const { ccclass, property } = cc._decorator;

@ccclass
export default class NewBankProxy {
    private linesDataArr: Array<NewBankLines> = [];//赔付线数据
    private _times: string;//局号
    private _gameOverData: NewBankGameOverNotify;//游戏结束数据
    private _betResponse: NewBankBetResponse;//下注返回数据
    private handlePayTableArr: Array<Array<number>> = [[], [], [], [], []];//表盘数据
    private _freeSpinCount: number = 0;//剩余免费转次数
    private _sceneStatus: NewBankSceneStatus = NewBankSceneStatus.NORMAL;//场景状态
    private _freeBet: number = 0;//免费转投注金额
    private addCircleStartIndex: number = -1;//需要加圈的起始下标（-1为不加圈）
    private _nextGameType: NewBankGameType = null;//下一局游戏类型
    private static _instance: NewBankProxy

    private _betGold: number = 1000;

    public static get instance() {
        if (NewBankProxy._instance == null) {
            NewBankProxy._instance = new NewBankProxy();
        }
        return NewBankProxy._instance;
    }

    private constructor() {
        this._gameOverData = new NewBankGameOverNotify();
        this._betResponse = new NewBankBetResponse();
    ...
    public set gameOverData(gameOverData) {
        ...
        NewBankSlotEventManager.instance.emit(NewBankEventDefine2.START_SPIN);
    }

    public randGameOverData(): void {
        this._gameOverData.times = '101';
        this._gameOverData.paytable = JSON.stringify({});
        let randNum = NewBankUtil.getRandomInt(0, 10);
        if (randNum < 5) {
            this._gameOverData.totalPay = 0;
        }
        else {
            randNum = NewBankUtil.getRandomInt(5, 20);
    ...
    public set freeSpinCount(count) {
        ...
        NewBankSlotEventManager.instance.emit(NewBankEventDefine2.UPDATE_FREE_COUNT);
    }
    ...
    public set sceneStatus(sceneStatus: NewBankSceneStatus) {
    ...
    public set nextGameType(type: NewBankGameType) {
    ...
    */
    public clean() {
        ...
        this.sceneStatus = NewBankSceneStatus.NORMAL;
    }
    ...
    private handleTable() {
        ...
        NewBankLogManager.instance.info("this.handlePayTableArr:", this.handlePayTableArr);
    }
        ...
        for (let index = 0; index < this.handlePayTableArr.length; index++) {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (payTableObj[num].scatter_circle != 0 && payTableObj[num].freegame[map[num]] > 0) {
                    ...
                    NewBankSlotEventManager.instance.emit(NewBankEventDefine2.UPDATE_CIRCLE, { index: index + 1, times: payTableObj[num].scatter_circle })
                    break;
                }
            }
            if (isAdd) {
    ...
    private setPayLines() {
        ...
        for (let index = 0; index < this._gameOverData.lineWin.length; index++) {
            ...
            this.linesDataArr.push(NewBankConfigProxy.instance.getConfigData('Level15_Lines')[lines.lineid] as NewBankLevel15Lines);
        }
    }
     ...
     * @returns 
     */
    public getPayLinesData(): Array<NewBankLines> {
