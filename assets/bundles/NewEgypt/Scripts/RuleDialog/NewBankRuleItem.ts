...
import LanguageRichText from "../BaseFrame/UI/Language/NewBankLanguageRichText";

const { ccclass, property } = cc._decorator;

@ccclass
export default class NewBankRuleItem extends cc.Component {
    @property(cc.Sprite)
    ...
    public async updateItem(data: any) {
        NewBankLogManager.instance.info("[NewBankRuleItem] updateItem data:", data);
        this.richTextConNode.removeAllChildren();
        //设置大图
        this.bgSprite.spriteFrame = null;
        let res: cc.SpriteFrame = null;
        if (data.pngName) {
            res = await NewBankSlotResManager.instance.loadByBundle(BundleType.STONE_AGE, "Textures/Bg/" + data.pngName, cc.SpriteFrame);
        }
        ...
        let atlas: cc.SpriteAtlas = await NewBankSlotResManager.instance.loadByBundle(BundleType.STONE_AGE, "Atlas/rules_txt_icon_stone_age", cc.SpriteAtlas);
