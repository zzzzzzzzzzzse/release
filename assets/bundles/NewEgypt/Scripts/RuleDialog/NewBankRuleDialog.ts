...
import PageViewEx from "../BaseFrame/UI/Component/NewBankPageViewEx";
import NewBankDialogBase from "../BaseFrame/UI/Dialog/NewBankDialogBase";

const { ccclass, property } = cc._decorator;

@ccclass
export default class NewBankRuleDialog extends NewBankDialogBase {
    @property(PageViewEx)
    ...
    public renderItem(item: cc.Node, idx: number) {
        // let tsNewBankRuleItem: NewBankRuleItem = <NewBankRuleItem>item.getComponent("NewBankRuleItem");
        // LogManager.instance.info("this.payTableArr:", this.payTableArr);
        // tsNewBankRuleItem.updateItem(this.payTableArr[idx]);
    ...
    private updateFooter(percent: number, index: number) {
        NewBankLogManager.instance.log("[NewBankRuleDialog] updateFooter:", percent, ",", index);
