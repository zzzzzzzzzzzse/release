import { SlotsStage } from "../../Const/NewBankCommonDefine";
...
@ccclass
export default class NewBankSlotWheelBase extends cc.Component {
    ...
    private onStop(): void {
        ...
        NewBankSlotEventManager.instance.emit(NewBankEventDefine.QUIE_SLOTS_ONE_STOP, { index: this.index, isCopy: this.isCopy });
    }
    ...
    public calcDistance(): void {
        if (this.time >= this.t0 + this.t1 + this.t2 + this.t3) {
            if (this.stage != SlotsStage.BOUNCE) {
                ...
                NewBankSlotEventManager.instance.emit(NewBankEventDefine.SLOTS_TURN_STAGE, { id: this.index, stage: this.stage });
                NewBankSlotEventManager.instance.emit(NewBankEventDefine.SLOTS_BOUNCE_START, { index: this.index })
            }
            ...
            if (this.time + this.uniformPhaseuUnitTs >= this.t0 + this.t1 + this.t2 && 0 == this.index) {
                NewBankLogManager.instance.info("检查：", this.curDistance, " index:", this.index);
                NewBankSlotEventManager.instance.emit(NewBankEventDefine.CHECK_SERVER_SLOT_DATA);
            }
        }
        else if (this.time >= this.t0) {
            if (this.stage != SlotsStage.SPEED_UP) {
                this.stage = SlotsStage.SPEED_UP;
                NewBankSlotEventManager.instance.emit(NewBankEventDefine.SLOTS_SPEED_UP_START, { index: this.index })
            }
            const t0 = this.t0;
        ...
        if (!this.isChangeDimImage && this.curDistance >= this.perDistance) {
            NewBankLogManager.instance.info("换图模糊：", this.index)
        ...
        if (!this.isChangeClearnessImage && this.curDistance >= this.distance - this.perDistance) {
            NewBankLogManager.instance.info("换图清晰：", this.index)
            this.isChangeClearnessImage = true;
            NewBankSlotEventManager.instance.emit(NewBankEventDefine.SLOTS_CHANGE_ICON_IMAGE, { isDim: false, id: this.index });
        }
        if (!this.isCollideBottom && this.curDistance >= (this.distance + this.debugX4)) {
            NewBankLogManager.instance.info("撞底：", this.index)
            this.isCollideBottom = true;
            NewBankSlotEventManager.instance.emit(NewBankEventDefine.WHEEL_COLLIDE_BOTTOM, { id: this.index });
        }
