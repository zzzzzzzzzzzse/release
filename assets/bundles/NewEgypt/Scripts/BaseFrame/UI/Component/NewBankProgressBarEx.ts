...
@ccclass
@menu('自定义组件/NewBankProgressBarEx')
export default class NewBankProgressBarEx extends NewBankComponentBase {
    @property({
    ...
    public setProgress(data: { current: number, total: number, isTween?: boolean, time?: number, overMaxIsClean?: boolean, easing?: string }) {
        ...
        data.current = NewBankUtil.getFloatToFixed(data.current, 0, false);
    ...
    public setProgressPercent(data: { percent: number, isTween?: boolean, time?: number, overMaxIsClean?: boolean, easing?: string }) {
        ...
        let current = NewBankUtil.getFloatToFixed(data.percent * 100, 0, false);
        let total = 100;
    ...
    private handle(width: number, curr: number, total: number) {
        ...
        switch (this.labelMode) {
            case LabelMode.PERCENT:
                this.txtLabel.string = NewBankUtil.getFloatToFixed(curr / total * 100, 0, false) + "%";
                break;
            case LabelMode.CURRENT_OR_TOTAL:
                this.txtLabel.string = NewBankUtil.getFloatToFixed(curr, 0, false) + "/" + total;
                break;
        }
    }
