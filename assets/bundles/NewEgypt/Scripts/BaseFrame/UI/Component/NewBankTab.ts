...
import NewBankListEx from "./NewBankListEx";
import NewBankTabItem from "./NewBankTabItem";

const { ccclass, property, menu } = cc._decorator;

@ccclass
@menu('自定义组件/Tab/NewBankTab')
export default class NewBankTab extends NewBankComponentBase {
    @property({
        type: [NewBankTabItem],
        visible() {
            return !this.virtual;
    ...
    private list: NewBankListEx = null;
    ...
    private virtualSelectChanged(): void {
        ...
        for (let i = 0; i < itemList.length; i++) {
            const tabItem = itemList[i].getComponent(NewBankTabItem);
        ...
        if (selectNode) {
            const selectItem = selectNode.getComponent(NewBankTabItem);
    ...
    private itemChanged(node: cc.Node, index: number): void {
        const tabItem = node.getComponent(NewBankTabItem);
        if (!tabItem) {
            NewBankLogManager.instance.warn(node.name + '必现继承TabItem');
            return;
        }
