import { BtnExStatus, ButtonMoveStatus } from "../../Const/NewBankCommonDefine";
import NewBankLogManager from "../../Manager/NewBankLogManager";
import { NewBankSlotAudioManager } from "../../Manager/NewBankSlotAudioManager";
...
@ccclass
@menu('自定义组件/NewBankButtonEx')
export default class NewBankButtonEx extends NewBankComponentBase {
    ...
    */
    private setLongPressTimer() {
        if (null != this._longPressCallback && !this.isLongPressRun) {
            this.longPressTimerId = NewBankTimerManager.instance.setInterval(() => {
                if (null == this._longPressCallback) {
                    return;
                }

                if (this.longPressMode == LongPressMode.CONTINUE) {
                    this.continueTimerId = NewBankTimerManager.instance.setInterval(() => {
            ...
            this.continueTimerId = NewBankTimerManager.instance.setInterval(this._longPressCallback, this.continueInterval);
        }
    }
    ...
    protected onClick() {
        if (this.cd) {
            this.cd = false;
            NewBankTimerManager.instance.setTimeOut(() => { this.cd = true; }, this.delay);
            if ("" != this.soundName) {
                NewBankSlotAudioManager.instance.playSound(this.aBName, this.soundName);
            }
    ...
    public setEnable() {
        ...
        for (let index = 0; index < this.grayLabelArr.length; index++) {
            const node = this.grayLabelArr[index];
            if (null != node) {
                let color = NewBankUtil.hexToRGB(parseInt(this.normalLabelColorArr[index]));
    ...
    public setDisable() {
        ...
        for (let index = 0; index < this.grayLabelArr.length; index++) {
            const node = this.grayLabelArr[index];
            if (null != node) {
                let color = NewBankUtil.hexToRGB(parseInt(this.grayLabelColorArr[index]));
                node.color = new cc.Color(color.r, color.g, color.b)
            }
        }
    }
     ...
     */
    private cleanTimer() {
        NewBankTimerManager.instance.clearInterval(this.continueTimerId);
        NewBankTimerManager.instance.clearInterval(this.longPressTimerId);
    }
    ...
    private scaleTouch() {
        ...
        if (1 == this.zoomScale) {
            NewBankLogManager.instance.err("[ButtonEx] 除数不能为0，无法缩放");
            return;
        }
    ...
    private normalPropertyChange() {
        for (let index = 0; index < this.normalOrDownPropertyChange.length; index++) {
            ...
            if (null == comp) {
                NewBankLogManager.instance.warn("[ButtonEx] normalPropertyChange ", arr[0], " is null");
                continue;
            }
            switch (arr[1]) {
                case "color":
                    let rgb = NewBankUtil.hexToRGB(parseInt(arr[2]));
    ...
    private downPropertyChange() {
        for (let index = 0; index < this.normalOrDownPropertyChange.length; index++) {
            ...
            if (null == comp) {
                NewBankLogManager.instance.warn("[ButtonEx] downPropertyChange ", arr[0], " is null");
                continue;
            }
            switch (arr[1]) {
                case "color":
                    let rgb = NewBankUtil.hexToRGB(parseInt(arr[3]));
                    comp[arr[1]] = new cc.Color(rgb.r, rgb.g, rgb.b);
                    break;
                default:
                    comp[arr[1]] = arr[3];
