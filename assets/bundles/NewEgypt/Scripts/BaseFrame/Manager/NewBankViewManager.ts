...
import { BundleType, ViewCloseAnimType, ViewLevel, ViewOpenAnimType } from "../Const/NewBankCommonDefine";
import NewBankViewBase from "../UI/View/NewBankViewBase";
...
@ccclass
export default class NewBankViewManager {
    ...
    private static _instance: NewBankViewManager
    public static get instance() {
        if (NewBankViewManager._instance == null) {
            NewBankViewManager._instance = new NewBankViewManager();
        }
        return NewBankViewManager._instance;
    }
    ...
    public openView(
        ...
        NewBankTimerManager.instance.clearInterval(this.delayReleaseIDMap[abundleName]);
    ...
    public back(
        ...
        for (let index = this.openListArr.length - 1; index >= 0; index--) {
            ...
            if (element.level == ViewLevel.LEVEL_ONE) {
                obj = element;
                NewBankLogManager.instance.log("openListArrBackOver：", this.openListArr);
                break;
            }
            else {
                ...
                NewBankLogManager.instance.log("openListArr：", this.openListArr);
            }
        }
        if (null == obj) {
            NewBankLogManager.instance.err("back view is null");
            return;
        }
        if (!isOpenLast) {
            return;
        }
        NewBankTimerManager.instance.clearInterval(this.delayReleaseIDMap[obj.abundleName]);
        this.opening(
            obj.abundleName,
            obj.path,
            obj.data,
    ...
    private handleView(
        ...
        let tsViewBase: NewBankViewBase = <NewBankViewBase>view.getComponent("ViewBase");
    ...
    private destroyLastView(isDestroy: boolean, closeAnimType: ViewCloseAnimType) {
        if (null != this.lastViewPath) {
            ...
            if (null != lastNode) {
                let tsOtherViewBase: NewBankViewBase = <NewBankViewBase>lastNode.getComponent("ViewBase");
                ...
                tsOtherViewBase.setClose(isDestroy, () => {
                    if (isGame && isDestroy) {
                        this.delayReleaseIDMap[_abundleName] = NewBankTimerManager.instance.setInterval(() => {
                            NewBankSlotResManager.instance.releaseBundle(_abundleName);
                            NewBankLogManager.instance.info("释放成功:", this.delayReleaseIDMap[_abundleName], " name:", _abundleName);
                        }, 0, 0);
                        NewBankLogManager.instance.info("开始释放：", _abundleName, "计时器：", this.delayReleaseIDMap[_abundleName]);
                    }
                }, closeAnimType);
    ...
    public returnHall() {
        ...
        this.clean();
        NewBankSlotResManager.instance.cleanGameBundle();
    ...
    public returnLogin() {
        ...
        this.clean();
        NewBankSlotResManager.instance.cleanAllBundle();
    ...
    private clean() {
        for (const key in this.viewMap) {
            ...
            if (null != element.node) {
                let tsOtherViewBase: NewBankViewBase = <NewBankViewBase>element.node.getComponent("ViewBase");
