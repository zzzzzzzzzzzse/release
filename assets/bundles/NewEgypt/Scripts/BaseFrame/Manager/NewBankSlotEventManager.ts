...
@ccclass
export default class NewBankSlotEventManager {
    private static _instance: NewBankSlotEventManager
    public static get instance() {
        if (NewBankSlotEventManager._instance == null) {
            NewBankSlotEventManager._instance = new NewBankSlotEventManager();
        }
        return NewBankSlotEventManager._instance;
    }
    ...
    public emit(type: string, data?: any) {
        let eventArr = this.eventMap[type];
        if (null == eventArr) {
            NewBankLogManager.instance.err(type, "event is null");
            return;
        }
        for (let index = 0; index < eventArr.length; index++) {
