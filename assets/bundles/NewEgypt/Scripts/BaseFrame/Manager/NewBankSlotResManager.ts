...
export default class NewBankSlotResManager {
    private static _instance: NewBankSlotResManager
    public static get instance() {
        if (NewBankSlotResManager._instance == null) {
            NewBankSlotResManager._instance = new NewBankSlotResManager();
        }
        return NewBankSlotResManager._instance;
    }
    ...
    public async loadBundle(aBName: string): Promise<cc.AssetManager.Bundle> {
        let startTime: number = NewBankUtil.getTimeStamp();
        let endTime: number;
        return new Promise<any>((resolve, reject) => {
            cc.assetManager.loadBundle(aBName, (err, bundle) => {
                if (err) {
                    NewBankLogManager.instance.log("loadBundleFail:", aBName);
                    reject(err.message || err);
                }
                else {
                    endTime = NewBankUtil.getTimeStamp();
                    NewBankLogManager.instance.log("loadBundleSucc:", aBName, " costTime:", endTime - startTime, "ms");
                    resolve(bundle);
                }
            });
        })
    ...
    public async loadByBundle(aBName: string, path: string, type?: typeof cc.Asset, onProgress?: (finish: number, total: number) => void): Promise<any> {
        ...
        type ||= cc.Asset;
        if (null == abundle) {
            NewBankLogManager.instance.err("abundle is null:", aBName);
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.load(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = NewBankUtil.getTimeStamp();
                        NewBankLogManager.instance.log("loadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        NewBankLogManager.instance.err(err)
                        reject(err.message || err);
                    }
                }
            );
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.loadDir(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = NewBankUtil.getTimeStamp();
                        NewBankLogManager.instance.log("loadDirByBundleSucc:", aBName, "->", dir, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        reject(err.message || err);
    ...
    public releaseBundle(aBName: string) {
        ...
        if (null != bundle) {
            ...
            NewBankLogManager.instance.log('释放bundle：', aBName);
            ...
            abundle.preloadDir(
                ...
                (err, asset) => {
                    if (null != err) {
                        ...
                        endTime = NewBankUtil.getTimeStamp();
                        NewBankLogManager.instance.log("preloadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                }
            );
