 ...
 * 计时器
 */
export default class NewBankTimerManager {
    private static _instance: NewBankTimerManager
    public static get instance() {
        if (NewBankTimerManager._instance == null) {
            NewBankTimerManager._instance = new NewBankTimerManager();
        }
        return NewBankTimerManager._instance;
    }
    ...
    public setInterval(callback: Function, interval: number = 0, repeat: number = null): number {
        let count = 0;
        if (repeat < 0) {
            NewBankLogManager.instance.err("Repeat must be equal or greater than 0");
            return;
        }
        if (null == repeat) {
            repeat = -1;
