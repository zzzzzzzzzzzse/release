import NewBankHttpManager from "./NewBankHttpManager";
import NewBankLogManager from "./NewBankLogManager";

export default class NewBankFileManager {
    private static _instance: NewBankFileManager = null;
     ...
     * 获取单例
     */
    public static get instance(): NewBankFileManager {
        if (!this._instance) {
            this._instance = new NewBankFileManager();
        }
    ...
    public downloadBinaryFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        if (count > this.maxCount) {
            ...
            NewBankLogManager.instance.log('达到最大下载数量，进入排队队列');
            return;
        }
    ...
    public downloadTxtFile(url: string, callback?: Function, timeOut: number = 30000): void {
        ...
        this.requestList[url] = callback;
        const download = url => {
            NewBankHttpManager.instance.sendRequestGet(url, response => {
                const callback = this.requestList[url];
    ...
    private downloadWaitList(): void {
        ...
        NewBankLogManager.instance.log('从队列中下载文件:', info.url);
        if (info.isBinary) {
