...
import NewBankDialogBase from "../UI/Dialog/NewBankDialogBase";
...
@ccclass
export default class NewBankDialogManager {
    ...
    private static _instance: NewBankDialogManager;
    public static get instance() {
        if (NewBankDialogManager._instance == null) {
            NewBankDialogManager._instance = new NewBankDialogManager();
        }
        return NewBankDialogManager._instance;
    }
    ...
    public openDialog(abundleName: string, path: string, data?: {}): void {
        if (this.isPopExist(path)) {
            NewBankLogManager.instance.log("重复打开弹框:", abundleName, "->", path);
        }
        else {
        ...
        if (null != this.dialogMap[path]) {
            NewBankLogManager.instance.log("弹框数据还在:", abundleName, "->", path);
            if (this.dialogMap[path].node != null) {
                NewBankLogManager.instance.log("弹框节点还在:", abundleName, "->", path);
                if (null == this.dialogCon) {
                    this.dialogCon = cc.find("Canvas/dialogNode");
                }
                let tsDialogBase: NewBankDialogBase = <NewBankDialogBase>this.dialogMap[path].node.getComponent("DialogBase");
            ...
            NewBankLogManager.instance.log("没有弹框数据:", abundleName, "->", path);
            this.dialogMap[path] = {};
    ...
    public closeDialog(path?: string, isDestroy: boolean = true) {
        ...
        let tsDialogBase: NewBankDialogBase = <NewBankDialogBase>dialogNode.getComponent("DialogBase");
    ...
    private async loadModule(abundleName: string, path: string, data?: {}) {
        NewBankLogManager.instance.log("开始加载弹框资源:", abundleName, "->", path);
        //这里放加载动画
        let res: cc.Prefab = await NewBankSlotResManager.instance.loadByBundle(
            abundleName,
            path,
            cc.Prefab,
            (finish, total) => {
        ...
        NewBankLogManager.instance.log("弹框资源加载成功:", abundleName, "->", path);
        //这里关闭加载动画
        var newNode = cc.instantiate(res);
        if (null == this.dialogCon) {
            this.dialogCon = cc.find("Canvas/dialogNode");
        }
        let tsDialogBase: NewBankDialogBase = <NewBankDialogBase>newNode.getComponent("DialogBase");
        tsDialogBase.setAbName(abundleName);
        tsDialogBase.setPath(path);
        tsDialogBase.setData(data);
    ...
    private isPopExist(path: string) {
        NewBankLogManager.instance.log("已有弹框：", this.openList);
        for (let index = 0; index < this.openList.length; index++) {
            const element = this.openList[index];
            if (element == path) {
                return true;
    ...
    public openCommonDialog(popupKey: string, confirmCallback: Function = null, cancelCallback: Function = null, replaceParams: any[] = []): void {
        ...
        NewBankDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonDialog', data);
    }
    ...
    public openCommonPrompts(titleLanguageKey: string, detailLanguageKey: string, wPos: cc.Vec2, params: any[] = [], refWPos: cc.Vec2 = null): void {
        ...
        NewBankDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonPrompts', data);
    }
    ...
    public closeCommonPrompts(): void {
        NewBankDialogManager.instance.closeDialog('Prefabs/Common/CommonPrompts');
    }
    ...
    public checkCommonDialogIsExist(): boolean {
        const dialogIsExist = NewBankDialogManager.instance.isPopExist('Prefabs/Common/CommonDialog');
    ...
    public closeAll() {
        while (0 != this.openList.length) {
            ...
            if (null == dialogNode) {
                ...
                NewBankLogManager.instance.info("closeDialog:", path);
            }
        }
    }
    ...
    public showCommonReward(itemList: any[]): void {
        ...
        NewBankDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonReward', itemList);
    }
    ...
    public showCommonItemList(itemList: any[], wPos: cc.Vec2): void {
        ...
        NewBankDialogManager.instance.openDialog(BundleType.RESOURCES, 'Prefabs/Common/CommonItemList', { itemList: itemList, wPos: wPos });
    }
}
