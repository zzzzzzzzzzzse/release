 ...
 * 动画管理
 */
export default class NewBankAnimationManager {
    private backPlayMap: { [key: number]: { animation: cc.Animation } } = {};//需要在切换到后台的时候依旧播放的动画数组
    private static _instance: NewBankAnimationManager;
    private id: number = 0;//每个动画的唯一id
    public static get instance() {
        if (NewBankAnimationManager._instance == null) {
            NewBankAnimationManager._instance = new NewBankAnimationManager();
        }
        return NewBankAnimationManager._instance;
    }

    public constructor() {
        NewBankSlotEventManager.instance.on(NewBankEventDefine.QUIE_GAME_SHOW, (interval: number) => {
    ...
    public async playAnimation(data: {
        ...
        let res: cc.Prefab = await NewBankSlotResManager.instance.loadByBundle(data.animBundle, data.animPath);
