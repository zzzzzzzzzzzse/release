...
export default abstract class NewBankProtocolBase {
    ...
    public async encode(data: any, msgName: string, needLog: boolean = true): Promise<any> {
        ...
        if (!this.pb || !this.pb[msgName]) {
            NewBankLogManager.instance.log(msgName + '协议未定义');
        }
        ...
        needLog && NewBankLogManager.instance.log('send===============' + msgName + '===============', data);
    ...
    public async decode(buffer: any, msgName: string, needLog: boolean = true) {
        ...
        needLog && NewBankLogManager.instance.log('recv===============' + msgName + '===============', data);
        return data;
    }
