...
import NewBankDialogManager from "../Manager/NewBankDialogManager";
...
export default class NewBankAppleSdk {
    private static _instance: NewBankAppleSdk = null;
     ...
     * 获取单例
     */
    public static get instance(): NewBankAppleSdk {
        if (!this._instance) {
            this._instance = new NewBankAppleSdk();
        }
    ...
    private notSupportLogin(): void {
        NewBankDialogManager.instance.openCommonDialog('cant_bind_apple');
    }
    ...
    private loginCallback(resultStr: string): void {
        ...
        NewBankSlotEventManager.instance.emit(NewBankEventDefine.APPLE_LOGIN_SUCCESS);
    }
    ...
    private buyItemsSuccess(resultStr: string): void {
        ...
        data['payId'] = payId;
        NewBankLogManager.instance.log(payId + '购买成功');
        if (this.isRestore) {
            this.isRestore = false;
