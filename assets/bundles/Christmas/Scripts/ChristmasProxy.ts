...
import { ChristmasGameType, ChristmasIconType, ChristmasSceneStatus } from "./ChristmasDefine";
import { ChristmasEventDefine2 } from "./ChristmasEventDefine2";
import ChristmasConfigProxy from "./Common/ChristmasConfigProxy";
import { ChristmasBetResponse, ChristmasGameOverNotify } from "./PBClass/ChristmasGame";
const { ccclass, property } = cc._decorator;

@ccclass
export default class ChristmasProxy {
    private linesDataArr: Array<ChristmasLines> = [];//赔付线数据
    private _times: string;//局号
    private _gameOverData: ChristmasGameOverNotify;//游戏结束数据
    private _betResponse: ChristmasBetResponse;//下注返回数据
    private handlePayTableArr: Array<Array<number>> = [[], [], [], [], []];//表盘数据
    private _freeSpinCount: number = 0;//剩余免费转次数
    private _sceneStatus: ChristmasSceneStatus = ChristmasSceneStatus.NORMAL;//场景状态
    private _freeBet: number = 0;//免费转投注金额
    private addCircleStartIndex: number = -1;//需要加圈的起始下标（-1为不加圈）
    private _nextGameType: ChristmasGameType = null;//下一局游戏类型
    private static _instance: ChristmasProxy

    private _betGold: number = 1000;

    public static get instance() {
        if (ChristmasProxy._instance == null) {
            ChristmasProxy._instance = new ChristmasProxy();
        }
        return ChristmasProxy._instance;
    }

    private constructor() {
        this._gameOverData = new ChristmasGameOverNotify();
        this._betResponse = new ChristmasBetResponse();
    ...
    public set gameOverData(gameOverData) {
        ...
        ChristmasSlotEventManager.instance.emit(ChristmasEventDefine2.START_SPIN);
    }

    public randGameOverData(): void {
        this._gameOverData.times = '101';
        this._gameOverData.paytable = JSON.stringify({});
        let randNum = ChristmasUtil.getRandomInt(0, 10);
        if (randNum < 5) {
            this._gameOverData.totalPay = 0;
        }
        else {
            randNum = ChristmasUtil.getRandomInt(5, 20);
    ...
    public set freeSpinCount(count) {
        ...
        ChristmasSlotEventManager.instance.emit(ChristmasEventDefine2.UPDATE_FREE_COUNT);
    }
    ...
    public set sceneStatus(sceneStatus: ChristmasSceneStatus) {
    ...
    public set nextGameType(type: ChristmasGameType) {
    ...
    */
    public clean() {
        ...
        this.sceneStatus = ChristmasSceneStatus.NORMAL;
    }
    ...
    private handleTable() {
        ...
        ChristmasLogManager.instance.info("this.handlePayTableArr:", this.handlePayTableArr);
    }
        ...
        for (let index = 0; index < this.handlePayTableArr.length; index++) {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (payTableObj[num].scatter_circle != 0 && payTableObj[num].freegame[map[num]] > 0) {
                    ...
                    ChristmasSlotEventManager.instance.emit(ChristmasEventDefine2.UPDATE_CIRCLE, { index: index + 1, times: payTableObj[num].scatter_circle })
                    break;
                }
            }
            if (isAdd) {
    ...
    private setPayLines() {
        ...
        for (let index = 0; index < this._gameOverData.lineWin.length; index++) {
            ...
            this.linesDataArr.push(ChristmasConfigProxy.instance.getConfigData('Level15_Lines')[lines.lineid] as ChristmasLevel15Lines);
        }
    }
     ...
     * @returns 
     */
    public getPayLinesData(): Array<ChristmasLines> {
