...
import ChristmasDialogManager from "../Manager/ChristmasDialogManager";
...
export default class ChristmasAppleSdk {
    private static _instance: ChristmasAppleSdk = null;
     ...
     * 获取单例
     */
    public static get instance(): ChristmasAppleSdk {
        if (!this._instance) {
            this._instance = new ChristmasAppleSdk();
        }
    ...
    private notSupportLogin(): void {
        ChristmasDialogManager.instance.openCommonDialog('cant_bind_apple');
    }
    ...
    private loginCallback(resultStr: string): void {
        ...
        ChristmasSlotEventManager.instance.emit(ChristmasEventDefine.APPLE_LOGIN_SUCCESS);
    }
    ...
    private buyItemsSuccess(resultStr: string): void {
        ...
        data['payId'] = payId;
        ChristmasLogManager.instance.log(payId + '购买成功');
        if (this.isRestore) {
            this.isRestore = false;
