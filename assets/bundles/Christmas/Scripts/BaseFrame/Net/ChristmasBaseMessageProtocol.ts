...
import ChristmasProtocolBase from "./ChristmasProtocolBase";

export default class ChristmasBaseMessageProtocol extends ChristmasProtocolBase {
    protected pbPackageName: string = "com.islet.game.proto";//当前协议包名
    protected pbName: string = "PB/BaseMessage";//pb文件名，不带扩展名

    private static _instance: ChristmasBaseMessageProtocol = null;
     ...
     * 获取单例
     */
    public static get instance(): ChristmasBaseMessageProtocol {
        if (!this._instance) {
            this._instance = new ChristmasBaseMessageProtocol();
        }
