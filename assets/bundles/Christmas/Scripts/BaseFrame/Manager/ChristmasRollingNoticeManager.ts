...
export default class ChristmasRollingNoticeManager {
    private static _instance: ChristmasRollingNoticeManager = null;
    ...
    public static get instance() {
        if (!this._instance) {
            this._instance = new ChristmasRollingNoticeManager();
        }
    ...
    private async showRollingNotice() {
        ...
        if (!this.rollingNoticeNode) {
            const prefab: cc.Prefab = await ChristmasSlotResManager.instance.loadByBundle(BundleType.HALL, 'Prefabs/RollingNotice/RollingNotice', cc.Prefab);
    ...
    public closeRollingNotice(loop: boolean = true): void {
        ...
        this.clear();
        loop && (this.timer = ChristmasTimerManager.instance.setTimeOut(this.showRollingNotice.bind(this), this.loopTime));
    }

    private clear(): void {
        ChristmasTimerManager.instance.clearTimeout(this.timer);
    }
}
