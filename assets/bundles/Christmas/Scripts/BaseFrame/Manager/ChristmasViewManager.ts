...
import { BundleType, ViewCloseAnimType, ViewLevel, ViewOpenAnimType } from "../Const/ChristmasCommonDefine";
import ChristmasViewBase from "../UI/View/ChristmasViewBase";
...
@ccclass
export default class ChristmasViewManager {
    ...
    private static _instance: ChristmasViewManager
    public static get instance() {
        if (ChristmasViewManager._instance == null) {
            ChristmasViewManager._instance = new ChristmasViewManager();
        }
        return ChristmasViewManager._instance;
    }
    ...
    public openView(
        ...
        ChristmasTimerManager.instance.clearInterval(this.delayReleaseIDMap[abundleName]);
    ...
    public back(
        ...
        for (let index = this.openListArr.length - 1; index >= 0; index--) {
            ...
            if (element.level == ViewLevel.LEVEL_ONE) {
                obj = element;
                ChristmasLogManager.instance.log("openListArrBackOver：", this.openListArr);
                break;
            }
            else {
                ...
                ChristmasLogManager.instance.log("openListArr：", this.openListArr);
            }
        }
        if (null == obj) {
            ChristmasLogManager.instance.err("back view is null");
            return;
        }
        if (!isOpenLast) {
            return;
        }
        ChristmasTimerManager.instance.clearInterval(this.delayReleaseIDMap[obj.abundleName]);
        this.opening(
            obj.abundleName,
            obj.path,
            obj.data,
    ...
    private handleView(
        ...
        let tsViewBase: ChristmasViewBase = <ChristmasViewBase>view.getComponent("ViewBase");
    ...
    private destroyLastView(isDestroy: boolean, closeAnimType: ViewCloseAnimType) {
        if (null != this.lastViewPath) {
            ...
            if (null != lastNode) {
                let tsOtherViewBase: ChristmasViewBase = <ChristmasViewBase>lastNode.getComponent("ViewBase");
                ...
                tsOtherViewBase.setClose(isDestroy, () => {
                    if (isGame && isDestroy) {
                        this.delayReleaseIDMap[_abundleName] = ChristmasTimerManager.instance.setInterval(() => {
                            ChristmasSlotResManager.instance.releaseBundle(_abundleName);
                            ChristmasLogManager.instance.info("释放成功:", this.delayReleaseIDMap[_abundleName], " name:", _abundleName);
                        }, 0, 0);
                        ChristmasLogManager.instance.info("开始释放：", _abundleName, "计时器：", this.delayReleaseIDMap[_abundleName]);
                    }
                }, closeAnimType);
    ...
    public returnHall() {
        ...
        this.clean();
        ChristmasSlotResManager.instance.cleanGameBundle();
    ...
    public returnLogin() {
        ...
        this.clean();
        ChristmasSlotResManager.instance.cleanAllBundle();
    ...
    private clean() {
        for (const key in this.viewMap) {
            ...
            if (null != element.node) {
                let tsOtherViewBase: ChristmasViewBase = <ChristmasViewBase>element.node.getComponent("ViewBase");
