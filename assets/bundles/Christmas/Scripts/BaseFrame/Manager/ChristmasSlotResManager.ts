...
export default class ChristmasSlotResManager {
    private static _instance: ChristmasSlotResManager
    public static get instance() {
        if (ChristmasSlotResManager._instance == null) {
            ChristmasSlotResManager._instance = new ChristmasSlotResManager();
        }
        return ChristmasSlotResManager._instance;
    }
    ...
    public async loadBundle(aBName: string): Promise<cc.AssetManager.Bundle> {
        let startTime: number = ChristmasUtil.getTimeStamp();
        let endTime: number;
        return new Promise<any>((resolve, reject) => {
            cc.assetManager.loadBundle(aBName, (err, bundle) => {
                if (err) {
                    ChristmasLogManager.instance.log("loadBundleFail:", aBName);
                    reject(err.message || err);
                }
                else {
                    endTime = ChristmasUtil.getTimeStamp();
                    ChristmasLogManager.instance.log("loadBundleSucc:", aBName, " costTime:", endTime - startTime, "ms");
                    resolve(bundle);
                }
            });
        })
    ...
    public async loadByBundle(aBName: string, path: string, type?: typeof cc.Asset, onProgress?: (finish: number, total: number) => void): Promise<any> {
        ...
        type ||= cc.Asset;
        if (null == abundle) {
            ChristmasLogManager.instance.err("abundle is null:", aBName);
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.load(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = ChristmasUtil.getTimeStamp();
                        ChristmasLogManager.instance.log("loadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        ChristmasLogManager.instance.err(err)
                        reject(err.message || err);
                    }
                }
            );
        ...
        return new Promise<any>((resolve, reject) => {
            abundle.loadDir(
                ...
                (err, asset) => {
                    if (!err) {
                        endTime = ChristmasUtil.getTimeStamp();
                        ChristmasLogManager.instance.log("loadDirByBundleSucc:", aBName, "->", dir, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                    else {
                        reject(err.message || err);
    ...
    public releaseBundle(aBName: string) {
        ...
        if (null != bundle) {
            ...
            ChristmasLogManager.instance.log('释放bundle：', aBName);
            ...
            abundle.preloadDir(
                ...
                (err, asset) => {
                    if (null != err) {
                        ...
                        endTime = ChristmasUtil.getTimeStamp();
                        ChristmasLogManager.instance.log("preloadByBundleSucc:", aBName, "->", path, " costTime:", endTime - startTime, "ms");
                        resolve(asset);
                    }
                }
            );
