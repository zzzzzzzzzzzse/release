 ...
 * 动画管理
 */
export default class ChristmasAnimationManager {
    private backPlayMap: { [key: number]: { animation: cc.Animation } } = {};//需要在切换到后台的时候依旧播放的动画数组
    private static _instance: ChristmasAnimationManager;
    private id: number = 0;//每个动画的唯一id
    public static get instance() {
        if (ChristmasAnimationManager._instance == null) {
            ChristmasAnimationManager._instance = new ChristmasAnimationManager();
        }
        return ChristmasAnimationManager._instance;
    }

    public constructor() {
        ChristmasSlotEventManager.instance.on(ChristmasEventDefine.QUIE_GAME_SHOW, (interval: number) => {
    ...
    public async playAnimation(data: {
        ...
        let res: cc.Prefab = await ChristmasSlotResManager.instance.loadByBundle(data.animBundle, data.animPath);
