...
export default class ChristmasHttpManager {
    private static _instance: ChristmasHttpManager = null;

    /**
     * 获取单例
     */
    public static get instance(): ChristmasHttpManager {
        if (!this._instance) {
            this._instance = new ChristmasHttpManager();
        }
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState == 4) {
                ...
                if (xhr.status >= 200 && xhr.status < 400) {
                    ...
                    ChristmasLogManager.instance.err(url + '请求失败, 内容为:', xhr.responseText);
    ...
    public sendRequestPost(url: string, data: any, callback?: (response: string) => void, timeOut: number = 3000) {
        ...
        setTimeout(() => {
            if (isTimeOut) {
                ChristmasLogManager.instance.err(url + '请求超时失败');
                xhr.abort();
            }
        }, xhr.timeout);
        return xhr;
