...
@ccclass
export default class ChristmasSlotEventManager {
    private static _instance: ChristmasSlotEventManager
    public static get instance() {
        if (ChristmasSlotEventManager._instance == null) {
            ChristmasSlotEventManager._instance = new ChristmasSlotEventManager();
        }
        return ChristmasSlotEventManager._instance;
    }
    ...
    public emit(type: string, data?: any) {
        let eventArr = this.eventMap[type];
        if (null == eventArr) {
            ChristmasLogManager.instance.err(type, "event is null");
            return;
        }
        for (let index = 0; index < eventArr.length; index++) {
