...
import ChristmasPageViewIndicator from "./ChristmasPageViewIndicator";
 ...
 */
export default class ChristmasPageViewEx extends ChristmasComponentBase {
    ...
    @property({ type: ChristmasPageViewIndicator, tooltip: '分页脚标' })
    private indicator: ChristmasPageViewIndicator = null;//分页脚标
    //渲染事件（渲染器）
    @property({
    ...
    private createItem(index: number, isBehind: boolean = true): void {
        ...
        ChristmasLogManager.instance.log('创建子项:', index);
    ...
    private checkLeftNeddRecover(): void {
        ...
        if (this.content.x + leftNodeInfo.node.x < -this.perDistance) {
            ...
            ChristmasLogManager.instance.log('回收子项:', leftNodeInfo.index);
        }
    }
    ...
    private checkRightNeddRecover(): void {
        ...
        if (this.content.x + rightNodeInfo.node.x > this.perDistance) {
            ...
            ChristmasLogManager.instance.log('回收子项:', rightNodeInfo.index);
    ...
    public jumpToPage(page: number): void {
        if (this.loop) {
            ChristmasLogManager.instance.log('循环分页不支持跳转到指定的页数');
            return;
        }
        if (page < 0 || page >= this.totalPage) {
            ChristmasLogManager.instance.log('跳转的页数请在0-' + (this.totalPage - 1));
            return;
        }
