...
import ChristmasListEx from "./ChristmasListEx";
import ChristmasTabItem from "./ChristmasTabItem";

const { ccclass, property, menu } = cc._decorator;

@ccclass
@menu('自定义组件/Tab/ChristmasTab')
export default class ChristmasTab extends ChristmasComponentBase {
    @property({
        type: [ChristmasTabItem],
        visible() {
            return !this.virtual;
        }
    })
    private items: ChristmasTabItem[] = [];
    @property({
        type: ChristmasListEx,
        visible() {
            return this.virtual;
        }
    })
    private list: ChristmasListEx = null;
    ...
    private virtualSelectChanged(): void {
        ...
        for (let i = 0; i < itemList.length; i++) {
            const tabItem = itemList[i].getComponent(ChristmasTabItem);
        ...
        if (selectNode) {
            const selectItem = selectNode.getComponent(ChristmasTabItem);
    ...
    private itemChanged(node: cc.Node, index: number): void {
        const tabItem = node.getComponent(ChristmasTabItem);
        if (!tabItem) {
            ChristmasLogManager.instance.warn(node.name + '必现继承TabItem');
            return;
        }
