...
import ChristmasPageViewIndicatorItem from "./ChristmasPageViewIndicatorItem";
...
export default class ChristmasPageViewIndicator extends ChristmasComponentBase {
    //模板类型
    ...
    public addItem(): void {
        ChristmasLogManager.instance.log('增加一个页标');
    ...
    public setPages(pages: number): void {
        ChristmasLogManager.instance.log('清空页标');
    ...
    public setCurrentPage(index: number = 0): void {
        ...
        for (let i = 0; i < this.itemList.length; i++) {
            ...
            const item = node.getComponent(ChristmasPageViewIndicatorItem);
