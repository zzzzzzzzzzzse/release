import ChristmasCircleScrollItem from "./ChristmasCircleScrollItem";
...
@ccclass
@menu('自定义组件/ChristmasCircleScroll')
export default class ChristmasCircleScroll extends cc.Component {
    ...
    private createItem(index: number, isRight: boolean = true): void {
        ...
        if (!nodeInfo) {
            ...
            const node = cc.instantiate(this.itemPrefab);
            const circleScrollItem = node.getComponent(ChristmasCircleScrollItem);
