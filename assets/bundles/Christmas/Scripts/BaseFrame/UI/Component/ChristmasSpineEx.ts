...
@ccclass
export default class ChristmasSpineEx extends sp.Skeleton {
    ...
    onDestroy() {
        if (BundleType.RESOURCES != this.abName) {
            ChristmasSlotResManager.instance.cleanAsset(this.abName, this.path, cc.SpriteAtlas);
        }
    }
    ...
    */
    public async playSpineAnim(data: {
        ...
        this.path = data.path;
        let res: sp.SkeletonData = await ChristmasSlotResManager.instance.loadByBundle(data.aBName, data.path, sp.SkeletonData);
        if (null == this.node || !this.node.isValid) {
            return;
        }
    ...
    */
    private mixSpine(_spine: sp.Skeleton, _mixTime: number) {
        ...
        ChristmasLogManager.instance.log("开始混合");
    ...
    public attachUtilSelf(targetNodeName: string, node: cc.Node) {
        ...
        if (null == boneNode) {
            ChristmasLogManager.instance.err("找不到挂点：", targetNodeName);
            return;
        }
