...
@ccclass
@menu('自定义组件/ChristmasLabelEx')
export default class ChristmasLabelEx extends ChristmasComponentBase {
    ...
    private onTicker(): void {
        ...
        if (!label) {
            ChristmasLogManager.instance.log('请将此节点挂在Label组建下');
            return;
        }
        if (this._ticker) {
    ...
    public digitEffect(data: {
        ...
        cc.tween(this.digitEffectObj)
            .to(data.time,
                ...
                {
                    onUpdate: () => {
                        let showNum = ChristmasUtil.getFloatToFixed(this.digitEffectObj.num, 0);
                        this.oldNum = showNum;
                        this.label.string = data.isCoin ?
                            data.isAbbr ? ChristmasUtil.formateCoinStrTwo(showNum.toString(), data.bit, data.isShowZero, data.unitBit, data.isShowSeparator) :
                                ChristmasUtil.formateCoinStr(showNum)
                            : showNum.toString();;
                    },
    ...
    private async createItem(index: number, data: RadianData): Promise<void> {
        ...
        if (null != data.font) {
            let atlas: cc.BitmapFont = await ChristmasSlotResManager.instance.loadByBundle(data.font.aBName, data.font.path, cc.BitmapFont);
            label.font = atlas;
            //@ts-ignore
    ...
    private getItemAngle(index: number, data: RadianData): number {
        ...
        if (null != data.startAngle) {
            ...
            if (null != data.specialSymbol) {
                let specialSymbolCount = ChristmasUtil.getCharCount(data.str, data.specialSymbol.symbol);
