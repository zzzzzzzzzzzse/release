...
import PageViewEx from "../BaseFrame/UI/Component/ChristmasPageViewEx";
import ChristmasDialogBase from "../BaseFrame/UI/Dialog/ChristmasDialogBase";

const { ccclass, property } = cc._decorator;

@ccclass
export default class ChristmasRuleDialog extends ChristmasDialogBase {
    @property(PageViewEx)
    ...
    public renderItem(item: cc.Node, idx: number) {
        // let tsChristmasRuleItem: ChristmasRuleItem = <ChristmasRuleItem>item.getComponent("ChristmasRuleItem");
        // LogManager.instance.info("this.payTableArr:", this.payTableArr);
        // tsChristmasRuleItem.updateItem(this.payTableArr[idx]);
    ...
    private updateFooter(percent: number, index: number) {
        ChristmasLogManager.instance.log("[ChristmasRuleDialog] updateFooter:", percent, ",", index);
