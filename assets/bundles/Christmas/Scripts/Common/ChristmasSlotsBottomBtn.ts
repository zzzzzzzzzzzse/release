...
import { BundleType, CommonTopStatus, GuideGroup, MaxStatus, SlotsViewScaleStatus, SoundsType, SpinAnimStatus, SpinBtnStatus } from "../BaseFrame/Const/ChristmasCommonDefine";
...
import ChristmasCommonTop from "./ChristmasCommonTop";
...
@ccclass
export default class ChristmasSlotsBottomBtn extends ChristmasComponentBase {
    @property(ButtonEx)
    ...
    private updateBetGold(): void {
        this.betLabelNode.getComponent(cc.Label).string = '' + ChristmasProxy.instance.betGold;
    }

    onDisable() {
        ChristmasLogManager.instance.log('清理SlotsBottomBtn的所有延迟调用');
        this.clearAllEvent();
    ...
    */
    ...
    public cancleNotOperationAnim() {
        ChristmasLogManager.instance.log("[SlotsBottomBtn] cancleNotOperationAnim");
    ...
    private onClickAddBet() {
        ...
        if (ChristmasProxy.instance.betGold >= 10000) {
            ChristmasProxy.instance.betGold = 1000;
            this.updateBetGold();
            return;
        }
        ChristmasProxy.instance.betGold += 1000;
    ...
    private onClickSubBet() {
        if (ChristmasProxy.instance.betGold <= 1000) {
            ChristmasProxy.instance.betGold = 10000;
            this.updateBetGold();
            return;
        }
        ChristmasProxy.instance.betGold -= 1000;
    ...
    private onLongPressSubBet() {
        ...
        ChristmasSlotEventManager.instance.emit(ChristmasEventDefine.BET_GOLD_CHANGE, { index: this.currBetRangeIndex });
    ...
    public onClickSpin(testData?: string) {
        ChristmasLogManager.instance.log("[SlotsBottomBtn] onClickSpin")
        if (!PlayerProxy.instance.roomId) {
            ChristmasLogManager.instance.err("房间已销毁，不能投注")
            return;
        }
        if (PlayerProxy.instance.gold < ChristmasProxy.instance.betGold) {
            return;
        }
        ...
        //先把钱扣了
        let tsCommonTop: ChristmasCommonTop = <ChristmasCommonTop>this.topNode.getComponent(ChristmasCommonTop);
        PlayerProxy.instance.gold -= ChristmasProxy.instance.betGold;
        tsCommonTop.updatePlayerGold(true, PlayerProxy.instance.gold);
        ChristmasProxy.instance.randGameOverData();
    ...
    public freeSpin() {
        ...
        //播放音效
        ChristmasSlotAudioManager.instance.playSound(BundleType.RESOURCES, SoundsType.SPIN);
        if (this.getSpinStatus() == SpinBtnStatus.SPIN) {
            this.stopSpinSpineAnim();
        }
        this.cancleNotOperationAnim();
   ...
   */
    ...
    */
    private onLongPressSpinEndCallback() {
        ChristmasLogManager.instance.log("[SlotsBottomBtn] onLongPressSpinEndCallback");
        // if (PlayerProxy.instance.gameId == parseInt(MessageType.CLASSIC777_MSG)) {
        //     const guideInfo = GuideProxy.instance.getCurrGuideInfo();
        //     if (guideInfo && guideInfo.group == GuideGroup.GUIDE_PLAY_GAME) {
        //         return;
        ...
        this.stopSpinSpineAnim();
        this.setNotOperationAnim();
        ChristmasDeviceManager.instance.stopShake();
        //回调
    ...
    public playSpinSpineAnim(animStatus?: SpinAnimStatus, repeat: number = 0, callback?: Function) {
        ...
        if (animStatus == SpinAnimStatus.NULL) {
            ChristmasLogManager.instance.log("stopBtnNull");
            this.stopSpinSpineAnim();
            return;
        }
    ...
    public setSpinStatus(status: SpinBtnStatus) {
        ...
        ChristmasLogManager.instance.log("设置按钮状态为：", status);
    ...
    public async setMaxBetStatus(status: MaxStatus) {
        ...
        switch (status) {
            case MaxStatus.MAX:
                ...
                betRes = await ChristmasSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt_max", cc.SpriteFrame);
            ...
            case MaxStatus.NOT_MAX:
                ...
                betRes = await ChristmasSlotResManager.instance.loadByBundle(BundleType.RESOURCES, "Textures/Common/SlotsBottom/s_bet_txt", cc.SpriteFrame);
                if (null != this.betTitleNode && this.betTitleNode.isValid) {
                    this.betTitleNode.getComponent(cc.Sprite).spriteFrame = betRes;
                }
                break;
    ...
    public setButtonCanNotClick() {
        ChristmasLogManager.instance.log("[SlotsBottomBtn] setButtonCanNotClick");
    ...
    public setButtonCanClick() {
        ChristmasLogManager.instance.log("[SlotsBottomBtn] setButtonCanClick");
    ...
    public setWinNum(data: { num: number, isChange: boolean, isBigWin?: boolean, bigWinTime?: number, initNum?: number }) {
        ...
        if (data.isChange) {
            ...
            this.winNumLabelNode.getComponent(ChristmasLabelEx).digitEffect({ score: data.num, time: time, bit: 0, isScale: true, initNum: data.initNum });
        }
        else {
            this.winNumLabelNode.getComponent(ChristmasLabelEx).stopDigitEffect();
