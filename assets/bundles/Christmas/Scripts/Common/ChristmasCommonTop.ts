...
import ProgressBarEx, { ProgressBarEvent } from "../BaseFrame/UI/Component/ChristmasProgressBarEx";
...
@ccclass
export default class ChristmasCommonTop extends ChristmasComponentBase {
    @property(ButtonEx)
    ...
    private goldLabel: ChristmasLabelEx = null;//金币
    @property(ChristmasLabelEx)
    private levelLabel: ChristmasLabelEx = null;//等级
    ...
    private addHandler(): void {
        ...
        ChristmasSlotEventManager.instance.on(ChristmasEventDefine.CLOSE_MENU_DIALOG, this.onMenuClose, this);
        ChristmasSlotEventManager.instance.on(ChristmasEventDefine.USERINFO_CHANGED, this.updateAvatar, this);
        ChristmasSlotEventManager.instance.on(ChristmasEventDefine.UPDATE_PLAYER_GOLD, this.playGoldChanged, this);
        ChristmasSlotEventManager.instance.on(ChristmasEventDefine.COMMON_TOP_STATUS_CHANGE, (status: CommonTopStatus) => {
    ...
    private onAvatar(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ChristmasTipsManager.instance.showTips(this.tipKey);
        }
        else {
            ChristmasDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/InformationDialog');
        }
    }
    ...
    private onShop(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            ChristmasViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/ShopView', null, this.isHall, ViewCloseAnimType.ONE, ViewOpenAnimType.ONE);
        }
    }
    ...
    private onVip(): void {
        if (PlayerProxy.instance.playerVO.gameState == CommonTopStatus.BAN) {
            ...
            ChristmasDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/PrimeDialog');
        }
    ...
    private onSaveMoney(): void {
        ...
        const unlock = ChristmasUtil.checkFunctionOpen(FunctionKey.BANK_PIG);
        if (!unlock) {
            return;
        }
        ChristmasViewManager.instance.openView(BundleType.HALL, 'Prefabs/View/BankView', null, this.isHall);
    }
    ...
    private onMenu(): void {
        this.showMenu = true;
        ChristmasDialogManager.instance.openDialog(BundleType.HALL, 'Prefabs/Dialog/MenuDialog', this.tipKey);
